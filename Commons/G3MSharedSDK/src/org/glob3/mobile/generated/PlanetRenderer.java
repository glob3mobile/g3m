...
public class PlanetRenderer
   ...
   private void visitTilesTouchesWith(final java.util.ArrayList<Layer> layers,
                                      final Sector sector,
                                      final int firstLevelToVisit,
                                      final int maxLevelToVisit) {
      ...
      if (_tileVisitor != null) {
         ...
         if (firstLevelToVisit == parameters._firstLevel) {
            ...
            logger.logInfo("%d request for precaching from %d to %d levels has been sent. Waiting responses...", numVisits,
                     firstLevelToVisit, maxLevelToVisit);
   ...
   private long visitSubTilesTouchesWith(final java.util.ArrayList<Layer> layers,
                                         final Tile tile,
      ...
      if (tile._level < maxLevel) {
         final java.util.ArrayList<Tile> subTiles = tile.getSubTiles(getLayerTilesRenderParameters()._mercator);
      ...
      if (_firstRender && _tilesRenderParameters._forceFirstLevelTilesRenderOnStart) {
         ...
         for (int i = 0; i < firstLevelTilesCount; i++) {
            ...
            tile.render(rc, _glState, null, planet, cameraNormalizedPosition, cameraAngle2HorizonInRadians,
                     ...
                     _layerSet, _renderedSector, _firstRender, _texturePriority, texWidthSquared, texHeightSquared, nowInMS,
                     _renderTileMeshes); // if first render, force full render
         }
      }
      else {
         ...
         while (toVisit.size() > 0) {
            ...
            for (final Tile tile : toVisit) {
               tile.render(rc, _glState, toVisitInNextIteration, planet, cameraNormalizedPosition, cameraAngle2HorizonInRadians,
                        ...
                        _tileRasterizer, _layerSet, _renderedSector, _firstRender, _texturePriority, texWidthSquared,
                        texHeightSquared, nowInMS, _renderTileMeshes); //SENDING SQUARED TEX SIZE -  if first render, force full render
            }
         ...
         if (_tilesRenderParameters._forceFirstLevelTilesRenderOnStart) {
            ...
            for (int i = 0; i < firstLevelTilesCount; i++) {
               ...
               tile.prepareForFullRendering(rc, _texturizer, _elevationDataProvider, _tessellator, _tileRasterizer,
                        layerTilesRenderParameters, _layerSet, _tilesRenderParameters, true, _texturePriority,
   ...
   public final void acceptTileVisitor(final ITileVisitor tileVisitor,
      ...
      if (tileVisitor != null) {
         ...
         for (int i = 0; i < layersCount; i++) {
            ...
            if (layer.isEnable() && (layer.getRenderState() == RenderState.ready())) {
               layers.add(layer);
            }
         }
