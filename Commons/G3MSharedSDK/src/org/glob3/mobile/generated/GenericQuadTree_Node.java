...
public class GenericQuadTree_Node {
   ...
   private final java.util.ArrayList<GenericQuadTree_Element> _elements = new java.util.ArrayList<GenericQuadTree_Element>();

   private GenericQuadTree_Node[]                             _children;


   private GenericQuadTree_Node(final Sector sector,
                                final GenericQuadTree_Node parent) {
   ...
   private void splitNode(final int maxElementsPerNode,
                          final int maxDepth,
                          final double childAreaProportion) {
      ...
      final Sector sector1 = new Sector(new Geodetic2D(lower._latitude, splitLongitude), new Geodetic2D(splitLatitude,
               upper._longitude));

      final Sector sector2 = new Sector(new Geodetic2D(splitLatitude, lower._longitude), new Geodetic2D(upper._latitude,
               splitLongitude));
      ...
      final java.util.ArrayList<GenericQuadTree_Element> elementsToBeInserted = _elements;
      _elements.clear();
   ...
   private GenericQuadTree_Node getBestNodeForInsertion(final GenericQuadTree_Element element,
                                                        final double childAreaProportion) {


      final Sector sector = element.getSector();
      //  double myArea = _sector->getAngularAreaInSquaredDegrees();
      final double myArea = getAreaInSquaredDegreesAfterInsertion(sector);
      ...
      for (int i = 1; i < 4; i++) {
         final GenericQuadTree_Node child = _children[i];
         final double cost = child.getInsertionCostInSquaredDegrees(sector);
         if (cost < minChildInsertionCost) {
            ...
            if (cost == minChildInsertionCost) {
               ...
               final int n1 = bestChildForInsertion.getSubtreeNElements();
               final int n2 = child.getSubtreeNElements();
   ...
   private void increaseNodeSector(final GenericQuadTree_Element element) {
      final Sector s = _sector;
      if (_sector != null) {
         _sector.dispose();
      }
   ...
   public GenericQuadTree_Node(final Sector sector) {
      _sector = new Sector(sector);
      _depth = 1;
      _children = null;
   }
   ...
   public final boolean add(final GenericQuadTree_Element element,
      ...
      final GenericQuadTree_Node bestInsertionNode = getBestNodeForInsertion(element, childAreaProportion);
   ...
   public final boolean acceptVisitor(final Sector sector,
                                      final GenericQuadTreeVisitor visitor) {
      ...
      for (int i = 0; i < elementsSize; i++) {
         final GenericQuadTree_Element element = _elements.get(i);

         if (element.isSectorElement()) //Element is associated to sector
         {
            final GenericQuadTree_SectorElement e = (GenericQuadTree_SectorElement) element;
         ...
         {
            final GenericQuadTree_Geodetic2DElement e = (GenericQuadTree_Geodetic2DElement) element;
   ...
   public final boolean acceptVisitor(final Geodetic2D geo,
                                      final GenericQuadTreeVisitor visitor) {
   ...
   public final boolean acceptNodeVisitor(final GenericQuadTreeNodeVisitor visitor) {
   ...
   public final double getInsertionCostInSquaredDegrees(final Sector sector) {

      final Sector newSector = _sector.mergedWith(sector);
      final double areaAfter = newSector.getAngularAreaInSquaredDegrees();

      final double areaNow = _sector.getAngularAreaInSquaredDegrees();
      return areaAfter - areaNow;
   }


   public final double getAreaInSquaredDegreesAfterInsertion(final Sector sector) {
      final Sector newSector = _sector.mergedWith(sector);
   ...
   public final void symbolize(final GEOVectorLayer geoVectorLayer) {

      if (_elements.size() > 0) {
         final java.util.ArrayList<Geodetic2D> coordinates = new java.util.ArrayList<Geodetic2D>();
         ...
         //float dashLengths = 0F;
         final int dashCount = 0;

         final Color c = Color.red().wheelStep(12, _depth);

         final GEO2DLineRasterStyle ls = new GEO2DLineRasterStyle(c, (float) 1.0, StrokeCap.CAP_ROUND, StrokeJoin.JOIN_ROUND, 1,
                  dashLengths, dashCount, 0); //const int dashPhase) : - const int dashCount, - float dashLengths[], - const float miterLimit, - const StrokeJoin join, -  const StrokeCap cap, - const float width, - const Color& color,


         final GEO2DCoordinatesData coordinatesData = new GEO2DCoordinatesData(coordinates);
         final GEOLineRasterSymbol symbol = new GEOLineRasterSymbol(coordinatesData, ls);
   ...
   public final void getGeodetics(final java.util.ArrayList<Geodetic2D> geo) {
   ...
   public final void getSectors(final java.util.ArrayList<Sector> sectors) {
      for (int i = 0; i < _elements.size(); i++) {
         if (_elements.get(i).isSectorElement()) {
