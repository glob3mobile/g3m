...
public abstract class MapBooBuilder {
   ...
   private final MapBoo_ViewType                 _viewType;

   private final MapBooApplicationChangeListener _applicationListener;
   ...
   private PlanetRenderer createPlanetRenderer() {
      ...
      final TileTessellator tessellator = new PlanetTileTessellator(skirted, Sector.fullSphere());

      final ElevationDataProvider elevationDataProvider = null;
      final float verticalExaggeration = 1F;
      final TileTexturizer texturizer = new MultiLayerTileTexturizer();
      final TileRasterizer tileRasterizer = null;
      ...
      final TilesRenderParameters parameters = new TilesRenderParameters(renderDebug, useTilesSplitBudget,
               forceFirstLevelTilesRenderOnStart, incrementalTileQuality, quality);

      final boolean showStatistics = false;
      final long texturePriority = DownloadPriority.HIGHER;
   ...
   private java.util.ArrayList<ICameraConstrainer> createCameraConstraints(final Planet planet,
                                                                           final PlanetRenderer planetRenderer) {
      final java.util.ArrayList<ICameraConstrainer> cameraConstraints = new java.util.ArrayList<ICameraConstrainer>();
   ...
   private java.util.ArrayList<PeriodicalTask> createPeriodicalTasks() {
      final java.util.ArrayList<PeriodicalTask> periodicalTasks = new java.util.ArrayList<PeriodicalTask>();
   ...
   private void recreateLayerSet() {
      ...
      if (scene == null) {
         ...
         final LayerSet newLayerSet = scene.createLayerSet();
   ...
   private Layer parseLayer(final JSONBaseObject jsonBaseObjectLayer) {
   ...
   private MapQuestLayer parseMapQuestLayer(final JSONObject jsonLayer,
   ...
   private BingMapsLayer parseBingMapsLayer(final JSONObject jsonLayer,
                                            final TimeInterval timeToCache) {
   ...
   private CartoDBLayer parseCartoDBLayer(final JSONObject jsonLayer,
                                          final TimeInterval timeToCache) {
   ...
   private MapBoxLayer parseMapBoxLayer(final JSONObject jsonLayer,
                                        final TimeInterval timeToCache) {
   ...
   private WMSLayer parseWMSLayer(final JSONObject jsonLayer) {
      ...
      final Sector sector = new Sector(new Geodetic2D(Angle.fromDegrees(lowerLat), Angle.fromDegrees(lowerLon)), new Geodetic2D(
               Angle.fromDegrees(upperLat), Angle.fromDegrees(upperLon)));
      final String imageFormat = jsonLayer.getAsString("imageFormat", "image/png");
      ...
      return new WMSLayer(mapLayer, mapServerURL, mapServerVersion, queryLayer, queryServerURL, queryServerVersion, sector,
               imageFormat, srs, style, isTransparent, null, timeToCache, readExpired, layerTilesRenderParameters);
   }


   private URLTemplateLayer parseURLTemplateLayer(final JSONObject jsonLayer) {
   ...
   private MapBoo_Scene parseScene(final JSONObject jsonObject) {
      if (jsonObject == null) {
         return null;
      }
      ...
      return new MapBoo_Scene(jsonObject.getAsString("id", ""), jsonObject.getAsString("name", ""), jsonObject.getAsString(
               "description", ""), parseMultiImage(jsonObject.getAsObject("screenshot")),
               parseColor(jsonObject.getAsString("backgroundColor")),
               parseCameraPosition(jsonObject.getAsObject("cameraPosition")), parseSector(jsonObject.get("sector")),
               parseLayer(jsonObject.get("baseLayer")), parseLayer(jsonObject.get("overlayLayer")), hasWarnings);
   }


   private Color parseColor(final JSONString jsonColor) {
      if (jsonColor == null) {
         return Color.black();
      }
      ...
      final Color result = new Color(color);
      if (color != null) {
         color.dispose();
      }
      return result;
   ...
   private Sector parseSector(final JSONBaseObject jsonBaseObjectLayer) {
      if (jsonBaseObjectLayer == null) {
         return null;
      }
   ...
   private MapBoo_MultiImage parseMultiImage(final JSONObject jsonObject) {
      if (jsonObject == null) {
         return null;
      }

      final Color averageColor = parseColor(jsonObject.getAsString("averageColor"));

      final java.util.ArrayList<MapBoo_MultiImage_Level> levels = new java.util.ArrayList<MapBoo_MultiImage_Level>();

      final JSONArray jsLevels = jsonObject.getAsArray("levels");
      if (jsLevels != null) {
         final int levelsCount = jsLevels.size();
         for (int i = 0; i < levelsCount; i++) {
            final MapBoo_MultiImage_Level level = parseMultiImageLevel(jsLevels.getAsObject(i));
            if (level != null) {
               levels.add(level);
            }
         }
   ...
   private MapBoo_MultiImage_Level parseMultiImageLevel(final JSONObject jsonObject) {
      ...
      return new MapBoo_MultiImage_Level(new URL(_serverURL, "/images/" + jsURL.value()), (int) jsWidth.value(),
               (int) jsHeight.value());
   }


   private MapBoo_CameraPosition parseCameraPosition(final JSONObject jsonObject) {
      if (jsonObject == null) {
         return null;
      }
      ...
      return new MapBoo_CameraPosition(Geodetic3D.fromDegrees(latitudeInDegress, longitudeInDegress, height),
               Angle.fromDegrees(headingInDegrees), Angle.fromDegrees(pitchInDegrees), animated);
   }
         ...
         if (currentScene != null) {
            ...
            if (cameraPosition != null) {
               if (cameraPosition.isAnimated()) {
                  _g3mWidget.setAnimatedCameraPosition(TimeInterval.fromSeconds(3), cameraPosition.getPosition(),
                           cameraPosition.getHeading(), cameraPosition.getPitch());
               }
               else {
   ...
   private String getSendNotificationCommand(final Geodetic2D position,
   ...
   private String toCameraPositionJSON(final Camera camera) {
      final IStringBuilder isb = IStringBuilder.newStringBuilder();
   ...
   private String toCameraPositionJSON(final MapBoo_CameraPosition cameraPosition) {
      final IStringBuilder isb = IStringBuilder.newStringBuilder();
   ...
   private MapBoo_Notification parseNotification(final JSONObject jsonObject) {
      if (jsonObject == null) {
         return null;
      }

      return new MapBoo_Notification(Geodetic2D.fromDegrees(jsonObject.getAsNumber("latitude", 0),
               jsonObject.getAsNumber("longitude", 0)), parseCameraPosition(jsonObject.getAsObject("cameraPosition")),
               jsonObject.getAsString("message", ""), parseURL(jsonObject.getAsString("iconURL")));
   }


   private java.util.ArrayList<MapBoo_Notification> parseNotifications(final JSONArray jsonArray) {
      final java.util.ArrayList<MapBoo_Notification> result = new java.util.ArrayList<MapBoo_Notification>();

      if (jsonArray != null) {
         final int size = jsonArray.size();
         for (int i = 0; i < size; i++) {
            final MapBoo_Notification notification = parseNotification(jsonArray.getAsObject(i));
   ...
   private void addApplicationNotification(final MapBoo_Notification notification) {
   ...
   private void addApplicationNotifications(java.util.ArrayList<MapBoo_Notification> notifications) {
      ...
      for (int i = 0; i < size; i++) {
         final MapBoo_Notification notification = notifications.get(i);
         if (notification != null) {
   ...
   private URL parseURL(final JSONString jsonString) {
      if (jsonString == null) {
         return null;
      }
   ...
   private void fireOnScenesChanged() {
      if (_applicationListener != null) {
         _applicationListener.onScenesChanged(_context, new java.util.ArrayList<MapBoo_Scene>(_applicationScenes));
      }
   }


   protected MapBooBuilder(final URL serverURL,
                           final URL tubesURL,
   ...
   protected final void setGL(final GL gl) {
      if (_gl != null) {
   ...
   protected final G3MWidget create() {
      ...
      final PlanetRenderer planetRenderer = createPlanetRenderer();
      ...
      final GInitializationTask initializationTask = new MapBooBuilder_ApplicationTubeConnector(this);
      ...
      final InitialCameraPositionProvider icpp = new SimpleInitialCameraPositionProvider();

      final Renderer hudRenderer = null;

      _g3mWidget = G3MWidget.create(getGL(), getStorage(), getDownloader(), getThreadUtils(), cameraActivityListener, planet,
               cameraConstraints, createCameraRenderer(), mainRenderer, createBusyRenderer(), createErrorRenderer(), hudRenderer,
               Color.black(), false, false, initializationTask, true, periodicalTasks, getGPUProgramManager(),
               createSceneLighting(), icpp); // autoDeleteInitializationTask -  logDownloaderStatistics -  logFPS
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void setApplicationTimestamp(final int timestamp) {
      _applicationTimestamp = timestamp;
   }


   /** Private to MapbooBuilder, don't call it */
   public final void setApplicationName(final String name) {
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void setApplicationWebsite(final String website) {
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void setApplicationEMail(final String eMail) {
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void setApplicationAbout(final String about) {
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void addApplicationScene(final MapBoo_Scene scene,
                                         final int position) {
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void deleteApplicationScene(final String sceneId) {
      final int scenesCount = _applicationScenes.size();
      int sceneIndex = -1;
      for (int i = 0; i < scenesCount; i++) {
      ...
      if (sceneIndex != -1) {
         final MapBoo_Scene scene = _applicationScenes.get(sceneIndex);
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void setApplicationScene(final MapBoo_Scene scene) {
      final int scenesCount = _applicationScenes.size();
      final String sceneToBeUpdatedID = scene.getId();
      for (int i = 0; i < scenesCount; i++) {
         final String sceneID = _applicationScenes.get(i).getId();
         if (sceneID.compareTo(sceneToBeUpdatedID) == 0) {
            final MapBoo_Scene oldScene = _applicationScenes.get(i);
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void setApplicationScenes(final java.util.ArrayList<MapBoo_Scene> applicationScenes) {
      final int currentScenesCount = _applicationScenes.size();
      for (int i = 0; i < currentScenesCount; i++) {
         final MapBoo_Scene scene = _applicationScenes.get(i);
         if (scene != null) {
            scene.dispose();
         }
      }
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void parseApplicationJSON(final String json,
                                          final URL url) {
         ...
         if (jsonObject == null) {
            ...
            if (jsonError == null) {
               ...
               if (getApplicationTimestamp() != timestamp) {
                  ...
                  if (jsonScene != null) {
                     final MapBoo_Scene scene = parseScene(jsonScene);
                     if (scene != null) {
                  ...
                  if (jsonAllScenes != null) {
                     final java.util.ArrayList<MapBoo_Scene> scenes = new java.util.ArrayList<MapBoo_Scene>();

                     final int scenesCount = jsonAllScenes.size();
                     for (int i = 0; i < scenesCount; i++) {
                        final MapBoo_Scene scene = parseScene(jsonAllScenes.getAsObject(i));
                        if (scene != null) {
                           scenes.add(scene);
                        }
                     }
                  ...
                  if (jsonScenes != null) {
                     ...
                     if (jsonPutScene != null) {
                        ...
                        final int position = (jsonPosition != null) ? (int) jsonPosition.value() : 0;
                        final JSONObject jsonNewScene = jsonPutScene.getAsObject("scene");
                        if (jsonNewScene != null) {
                           final MapBoo_Scene scene = parseScene(jsonNewScene);
                           if (scene != null) {
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void openApplicationTube(final G3MContext context) {
      ...
      _webSocket = factory.createWebSocket(createApplicationTubeURL(), new MapBooBuilder_ApplicationTubeListener(this), true,
               true); // autodeleteWebSocket -  autodeleteListener
   }


   /** Private to MapbooBuilder, don't call it */
   public final void setApplicationCurrentSceneId(final String currentSceneId) {
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void rawChangeScene(final String sceneId) {
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void setContext(final G3MContext context) {
      _context = context;
   }


   /** Private to MapbooBuilder, don't call it */
   public final void setApplicationTubeOpened(final boolean open) {
   ...
   /** Private to MapbooBuilder, don't call it */
   public final boolean onTerrainTouch(final G3MEventContext ec,
                                       final Vector2I pixel,
                                       final Camera camera,
   ...
   public final MapBoo_Notification createNotification(final Geodetic2D position,
                                                       final Camera camera,
                                                       final String message,
                                                       final URL iconURL) {
      final MapBoo_CameraPosition cameraPosition = new MapBoo_CameraPosition(camera.getGeodeticPosition(), camera.getHeading(),
               camera.getPitch(), true); // animated
      return new MapBoo_Notification(position, cameraPosition, message, iconURL);
   }


   public final void sendNotification(final Geodetic2D position,
                                      final MapBoo_CameraPosition cameraPosition,
                                      final String message,
                                      final URL iconURL) {
      if ((_webSocket != null) && _isApplicationTubeOpen) {
   ...
   public final void changeScene(final String sceneId) {
      final String currentSceneId = getApplicationCurrentSceneId();
   ...
   public final void changeScene(final MapBoo_Scene scene) {
   ...
   public final URL createGetFeatureInfoRestURL(final Tile tile,
                                                final Vector2I size,
                                                final Vector2I pixel,
                                                final Geodetic3D position) {
      final IStringBuilder isb = IStringBuilder.newStringBuilder();
