...
public abstract class MapBooBuilder {
   ...
   private PlanetRenderer createPlanetRenderer() {
      ...
      final TilesRenderParameters parameters = new TilesRenderParameters(renderDebug, useTilesSplitBudget,
               forceFirstLevelTilesRenderOnStart, incrementalTileQuality);

      final boolean showStatistics = false;
      final long texturePriority = DownloadPriority.HIGHER;

      final int TODO_CHECK_MAPBOO_FULLSPHERE;
      final Sector renderedSector = Sector.fullSphere();

      return new PlanetRenderer(tessellator, elevationDataProvider, verticalExaggeration, texturizer, tileRasterizer, _layerSet,
               parameters, showStatistics, texturePriority, renderedSector);
   }
   ...
   private WMSLayer parseWMSLayer(final JSONObject jsonLayer) {
      ...
      return new WMSLayer(mapLayer, mapServerURL, mapServerVersion, queryLayer, queryServerURL, queryServerVersion, sector,
               imageFormat, (srs.compareTo("EPSG_4326") == 0) ? "EPSG:4326" : "EPSG:900913", style, isTransparent, null,
               timeToCache, readExpired, layerTilesRenderParameters);
   }
   ...
   private MapBoo_Scene parseScene(final JSONObject jsonObject) {
      ...
      return new MapBoo_Scene(jsonObject.getAsString("name", ""), jsonObject.getAsString("description", ""),
               parseMultiImage(jsonObject.getAsObject("screenshot")), parseColor(jsonObject.getAsString("backgroundColor")),
               parseCameraPosition(jsonObject.getAsObject("cameraPosition")), parseLayer(jsonObject.get("baseLayer")),
               parseLayer(jsonObject.get("overlayLayer")), hasWarnings);
   }
   ...
   protected final G3MWidget create() {
      ...
      _g3mWidget = G3MWidget.create(getGL(), getStorage(), getDownloader(), getThreadUtils(), cameraActivityListener, planet,
               cameraConstraints, createCameraRenderer(), mainRenderer, createBusyRenderer(), Color.black(), false, false,
               initializationTask, true, periodicalTasks, getGPUProgramManager(), createSceneLighting(), icpp); // autoDeleteInitializationTask -  logDownloaderStatistics -  logFPS
               ...
               if (getApplicationTimestamp() != timestamp) {
                  ...
                  if (jsonScenes != null) {
                     ...
                     for (int i = 0; i < scenesCount; i++) {
                        final MapBoo_Scene scene = parseScene(jsonScenes.getAsObject(i));
                        if (scene != null) {
                           scenes.add(scene);
                        }
                     }
   ...
   public final void openApplicationTube(final G3MContext context) {
      ...
      downloader.requestBuffer(createApplicationRestURL(), DownloadPriority.HIGHEST, TimeInterval.zero(), false,
               new MapBooBuilder_RestJSON(this), true); // readExpired
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void setApplicationCurrentSceneIndex(final int sceneIndex) {
   ...
   /** Private to MapbooBuilder, don't call it */
   public final void rawChangeScene(final int sceneIndex) {
      _applicationCurrentSceneIndex = sceneIndex;
   ...
   public final void changeScene(final int sceneIndex) {
