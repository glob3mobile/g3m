...
//ORIGINAL LINE: boolean equalTo(const RectangleF& that) const
...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: boolean fullContains(const RectangleF& that) const
...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: boolean contains(float x, float y) const
...
//ORIGINAL LINE: if (wholeSector.isEquals(innerSector))
	if (wholeSector.isEquals(new Sector(innerSector)))
	{
	  return new RectangleF(0, 0, wholeSectorWidth, wholeSectorHeight);
	}
