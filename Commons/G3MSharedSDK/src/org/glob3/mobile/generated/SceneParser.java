...
public class SceneParser {
   ...
   private void parserJSONWMSLayer(final LayerSet layerSet,
      ...
      final boolean transparent = getValueBooleanParam(jsonLayer.getAsString(ISTRANSPARENT));

      final String format = getFormat(transparent);

      final LevelTileCondition levelTileCondition = getLevelCondition(jsonLayer.getAsString(MINLEVEL),
               jsonLayer.getAsString(MAXLEVEL));
      final Sector sector = getSector(jsonLayer.getAsObject(BBOX));
      ...
      final WMSLayer wmsLayer = new WMSLayer(URL.escape(layersSecuence), new URL(jsonURL, false), wmsVersion, sector, format,
               "EPSG:4326", "", transparent, levelTileCondition, TimeInterval.fromDays(30), true, new LayerTilesRenderParameters(
   ...
   private void parserJSONTMSLayer(final LayerSet layerSet,
                                   final JSONObject jsonLayer) {
      ...
      final boolean enabled = getValueBooleanParam(jsonLayer.getAsString(ISENABLED));
   ...
   private LevelTileCondition getLevelCondition(final JSONString jsonMinLevel,
                                                final JSONString jsonMaxLevel) {
   ...
   private Sector getSector(final JSONObject jsonBBOX) {
      if (jsonBBOX != null) {
         ...
         if ((minx < maxx) && (miny < maxy)) {
            return new Sector(new Geodetic2D(Angle.fromDegrees(miny), Angle.fromDegrees(minx)), new Geodetic2D(
                     Angle.fromDegrees(maxy), Angle.fromDegrees(maxx)));
   ...
   private boolean getValueBooleanParam(final JSONString param) {
   ...
   private String getFormat(final boolean transparent) {
