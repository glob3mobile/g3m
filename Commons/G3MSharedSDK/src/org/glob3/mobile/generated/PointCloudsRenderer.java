...
public class PointCloudsRenderer
   ...
   private abstract static class PointCloudNode
      ...
      protected PointCloudNode(final String id) {
         _id = id;
         _rendered = false;
         _projectedArea = -1;
      ...
      protected abstract long rawRender(PointCloud pointCloud,
                                        G3MRenderContext rc,
                                        GLState glState,
                                        Frustum frustum,
                                        ...
                                        float pointSize,
                                        long nowInMS,
                                        boolean justRecalculatedProjectedArea);
      ...
      public final long render(final PointCloud pointCloud,
         ...
         if (bounds != null) {
            if (bounds.touchesFrustum(frustum)) {
               ...
               if (_projectedArea >= minProjectedArea) {
                  final long renderedCount = rawRender(pointCloud, rc, glState, frustum, _projectedArea, minHeight, maxHeight,
                           pointSize, nowInMS, justRecalculatedProjectedArea);
                  _rendered = true;
   ...
   private static class PointCloudInnerNode
            extends
               PointCloudNode {
      private final PointCloudNode[] _children = new PointCloudNode[4];
      ...
      private Box calculateBounds() {
         Box bounds = null;
         for (int i = 0; i < 4; i++) {
            final PointCloudNode child = _children[i];
            if (child != null) {
      ...
      @Override
      protected final long rawRender(final PointCloud pointCloud,
                                     final G3MRenderContext rc,
                                     final GLState glState,
                                     final Frustum frustum,
                                     ...
                                     final float pointSize,
                                     final long nowInMS,
                                     final boolean justRecalculatedProjectedArea) {
         ...
         if (renderedCount == 0) {
            if (_mesh == null) {
               ...
               final IFloatBuffer pointsBuffer = rc.getFactory().createFloatBuffer(3);
               ...
               _mesh = new DirectMesh(GLPrimitive.points(), true, new Vector3D(averageX, averageY, averageZ), pointsBuffer, 1,
                        pointSize * 2, Color.newFromRGBA(1, 1, 0, 1), null, 1, true); // colorsIntensity -  colors -  flatColor
            }
      ...
      public PointCloudInnerNode(final String id) {
         super(id);
         _bounds = null;
         _average = null;
         _pointsCount = -1;
      ...
      public final void addLeafNode(final PointCloudLeafNode leafNode) {
      ...
      public final PointCloudsRenderer.PointCloudInnerNode pruneUnneededParents() {
         ...
         if (onlyChild != null) {
            if (onlyChild.isInner()) {
               final PointCloudInnerNode result = ((PointCloudInnerNode) onlyChild).pruneUnneededParents();
      ...
      @Override
      public final void stoppedRendering(final G3MRenderContext rc) {
         if (_mesh != null) {
            _mesh.dispose();
         }
         _mesh = null;
   ...
   private static class PointCloudLeafNodeLevelParserTask
            extends
               GAsyncTask {
      private final PointCloudLeafNode _leafNode;
      ...
      public PointCloudLeafNodeLevelParserTask(final PointCloudLeafNode leafNode,
                                               final int level,
   ...
   private static class PointCloudLeafNodeLevelListener
            extends
      ...
      public PointCloudLeafNodeLevelListener(final PointCloudLeafNode leafNode,
                                             final int level,
   ...
   private static class PointCloudLeafNode
            extends
               PointCloudNode {
      ...
      private final IFloatBuffer _firstPointsVerticesBuffer;
      private final IFloatBuffer _firstPointsHeightsBuffer;
      ...
      private final int          _preloadedLevel;
      ...
      private final IFloatBuffer[] _levelsVerticesBuffers;
      private final IFloatBuffer[] _levelsHeightsBuffers;


      private DirectMesh createMesh(final double minHeight,
                                    final double maxHeight,
                                    final float pointSize) {
         ...
         if (_currentLoadedLevel <= _preloadedLevel) {
            ...
            final DirectMesh mesh = new DirectMesh(GLPrimitive.points(), false, _average, _firstPointsVerticesBuffer, 1,
                     pointSize, null, _firstPointsColorsBuffer, 1, true); // colorsIntensity -  colors -  flatColor
         ...
         final IFloatBuffer vertices = IFactory.instance().createFloatBuffer(pointsCount * 3);
         ...
         for (int level = _preloadedLevel + 1; level <= _currentLoadedLevel; level++) {
            final IFloatBuffer levelVerticesBuffers = _levelsVerticesBuffers[level];
            vertices.rawPut(cursor, levelVerticesBuffers);

            cursor += levelVerticesBuffers.size();
         }

         final IFloatBuffer colors = IFactory.instance().createFloatBuffer(pointsCount * 4);
         final double deltaHeight = maxHeight - minHeight;
         final int firstPointsCount = firstPointsVerticesBufferSize / 3;
         ...
         for (int level = _preloadedLevel + 1; level <= _currentLoadedLevel; level++) {
            final IFloatBuffer levelHeightsBuffers = _levelsHeightsBuffers[level];
         ...
         final DirectMesh mesh = new DirectMesh(GLPrimitive.points(), true, _average, vertices, 1, pointSize, null, colors, 1,
                  true); // colorsIntensity -  colors -  flatColor
         // mesh->setRenderVerticesCount( mu->min(_neededPoints, firstPointsCount) );
         ...
         if ((_loadingLevel >= 0) && (_neededLevel < _loadingLevel) && (_loadingLevelRequestID >= 0)) {
            ...
            if (_neededLevel > _currentLoadedLevel) {
               if (_loadingLevel < 0) {
                  ...
                  _loadingLevelRequestID = pointCloud.requestBufferForLevel(rc, _id, _loadingLevel,
                           new PointCloudLeafNodeLevelListener(this, _loadingLevel, rc.getThreadUtils()), true);
               }
            }
      ...
      public final void onLevelBuffersDownload(final int level,
                                               final IFloatBuffer verticesBuffer,
                                               final IFloatBuffer heightsBuffer) {
         _loadingLevel = -1;
         _loadingLevelRequestID = -1;
      ...
      public final void onLevelBufferError(final int level) {
         _loadingLevel = -1;
         _loadingLevelRequestID = -1;
      }


      public final void onLevelBufferCancel(final int level) {
         _loadingLevel = -1;
         _loadingLevelRequestID = -1;
      }
   ...
   private static class PointCloudMetadataParserAsyncTask
      ...
      private PointCloudsRenderer.PointCloudLeafNode parseLeafNode(final ByteBufferIterator it) {
         ...
         final int levelsCountLength = byteLevelsCount + shortLevelsCount + intLevelsCount;

         final int[] levelsCount = new int[levelsCountLength];
         ...
         final IFloatBuffer firstPointsVerticesBuffer = IFactory.instance().createFloatBuffer(firstPointsCount * 3);
         for (int i = 0; i < firstPointsCount; i++) {
         ...
         final IFloatBuffer firstPointsHeightsBuffer = IFactory.instance().createFloatBuffer(firstPointsCount);
         for (int i = 0; i < firstPointsCount; i++) {
            firstPointsHeightsBuffer.rawPut(i, it.nextFloat());
         }

         return new PointCloudLeafNode(id, levelsCountLength, levelsCount, average, bounds, firstPointsVerticesBuffer,
      ...
      public final void runInBackground(final G3MContext context) {
         ...
         final java.util.ArrayList<PointCloudLeafNode> leafNodes = new java.util.ArrayList<PointCloudLeafNode>();
   ...
   private static class PointCloud {
      ...
      public final void initialize(final G3MContext context) {
         ...
         final URL metadataURL = new URL(_serverURL, _cloudName + "?planet=" + planetType + "&verticalExaggeration="
                                                     + IStringUtils.instance().toString(_verticalExaggeration) + "&format=binary");
      ...
      public final void parsedMetadata(final long pointsCount,
                                       ...
                                       final PointCloudInnerNode rootNode) {
      ...
      public final void render(final G3MRenderContext rc,
         ...
         if (_rootNode != null) {
            //C++ TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
            //#warning TODO
            final long renderedCount = _rootNode.render(this, rc, glState, frustum, _minHeight, _averageHeight * 3, _pointSize,
                     nowInMS);
      ...
      public final long requestBufferForLevel(final G3MRenderContext rc,
                                              final String nodeID,
                                              final int level,
         ...
         final URL url = new URL(_serverURL, _cloudName + "/" + nodeID + "/" + IStringUtils.instance().toString(level)
                                             + "?planet=" + planetType + "&verticalExaggeration="
                                             + IStringUtils.instance().toString(_verticalExaggeration) + "&format=binary");

         //  ILogger::instance()->logInfo("Downloading metadata for \"%s\"", _cloudName.c_str());

         return rc.getDownloader().requestBuffer(url, _downloadPriority - level, _timeToCache, _readExpired, listener,
                  deleteListener);
      }
   ...
   public final void addPointCloud(final URL serverURL,
                                   ...
                                   final PointCloudMetadataListener metadataListener,
                                   final boolean deleteListener) {
      addPointCloud(serverURL, cloudName, DownloadPriority.MEDIUM, TimeInterval.fromDays(30), true, pointSize,
               verticalExaggeration, metadataListener, deleteListener);
   }
   ...
   public final void addPointCloud(final URL serverURL,
                                   ...
                                   final float verticalExaggeration,
                                   final PointCloudMetadataListener metadataListener) {
      addPointCloud(serverURL, cloudName, downloadPriority, timeToCache, readExpired, pointSize, verticalExaggeration,
   ...
   public final void addPointCloud(final URL serverURL,
                                   ...
                                   final PointCloudMetadataListener metadataListener,
                                   final boolean deleteListener) {
      final PointCloud pointCloud = new PointCloud(serverURL, cloudName, verticalExaggeration, pointSize, downloadPriority,
               timeToCache, readExpired, metadataListener, deleteListener);
      if (_context != null) {
