...
//ORIGINAL LINE: static void drawStringsOn(const java.util.ArrayList<String>& strings, ICanvas *canvas, const int width, const int height, const HorizontalAlignment hAlign, const VerticalAlignment vAlign, const HorizontalAlignment textAlign, const Color& color, const int maxFontSize = 18, const int minFontSize = 2, const Color& backgroundColor = Color::transparent(), const Color& shadowColor = Color::black(), const int padding = 16, const int cornerRadius = 8)
...
//ORIGINAL LINE: canvas->setShadow(shadowColor, 1.0f, 0.0f, 0.0f);
	canvas.setShadow(new Color(shadowColor), 1.0f, 0.0f, 0.0f);
...
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: const Vector2F position = getPosition(extent, width, height, hAlign, vAlign);
	final Vector2F position = getPosition(new Vector2F(extent), width, height, hAlign, vAlign);
