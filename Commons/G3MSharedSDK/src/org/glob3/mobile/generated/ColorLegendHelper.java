...
//#warning TODO CHECK CJenksBreaks
  ...
  public static ColorLegend createColorBrewLegendWithHomogeneousBreaks(java.util.ArrayList<Double> sListDouble, String colorName, int sClassCount)
  {
  
    java.util.ArrayList<Double> naturalBreaks = new java.util.ArrayList<Double>();
  
    std.sort(sListDouble.iterator(), sListDouble.end());
  
    final double size = (double)sListDouble.size();
    ...
    final double begin = q1 - 1.5 * iqr > sListDouble.get(0) != 0? q1 - 1.5 * iqr : sListDouble.get(0);
    final double end = q3 + 1.5 * iqr < sListDouble.get(sListDouble.size()-1) != 0? q3 + 1.5 * iqr : sListDouble.get(sListDouble.size()-1);
