...
public class ColorLegendHelper
{
  ...
  public static ColorLegend createColorBrewLegendWithNaturalBreaks(tangible.RefObject<java.util.ArrayList<Double>> sListDouble, String colorName, int sClassCount)
  {
//C++ TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
//#warning TODO CHECK CJenksBreaks
	CJenksBreaks jenks = new CJenksBreaks(sListDouble.argvalue, sClassCount);
  ...
  public static ColorLegend createColorBrewLegendWithHomogeneousBreaks(tangible.RefObject<java.util.ArrayList<Double>> sListDouble, String colorName, int sClassCount)
  {
  
	java.util.ArrayList<Double> naturalBreaks = new java.util.ArrayList<Double>();
  
	std.sort(sListDouble.argvalue.iterator(), sListDouble.argvalue.end());
  
	final double size = (double)sListDouble.argvalue.size();
	final double q1 = sListDouble.argvalue.get((int)IMathUtils.instance().floor(size * 0.25));
	final double q3 = sListDouble.argvalue.get((int)IMathUtils.instance().floor(size * 0.75));
	final double iqr = q3-q1;
  
	final double begin = q1 - 1.5 * iqr > sListDouble.argvalue.get(0) != 0? q1 - 1.5 * iqr : sListDouble.argvalue.get(0);
	final double end = q3 + 1.5 * iqr < sListDouble.argvalue.get(sListDouble.argvalue.size()-1) != 0? q3 + 1.5 * iqr : sListDouble.argvalue.get(sListDouble.argvalue.size()-1);
  ...
  //  const double end = sListDouble.at((size_t)IMathUtils::instance()->floor( size * 0.8 )); //Removing outliers
	final double gap = (end-begin) / sClassCount;
	for (int i = 0; i < sClassCount; i++)
	{
	  naturalBreaks.add(begin + i * gap);
	}
  ...
  public static ColorLegend createColorBrewLegendWithEquallySpacedBreaks(tangible.RefObject<java.util.ArrayList<Double>> sListDouble, String colorName, int sClassCount)
  {
	double min = sListDouble.argvalue.get(0);
	double max = sListDouble.argvalue.get(0);
  
	for (int i = 1; i < sListDouble.argvalue.size(); i++)
	{
	  double v = sListDouble.argvalue.get(i);
	  if (v < min)
	  {
		min = v;
	  }
