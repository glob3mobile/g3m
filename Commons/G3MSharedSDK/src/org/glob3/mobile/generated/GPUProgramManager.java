...
public class GPUProgramManager {
   ...
   private final java.util.HashMap<String, IGPUProgram> _programs = new java.util.HashMap<String, IGPUProgram>();


   //GPUProgramFactory *_factory;

   private IGPUProgram getCompiledProgram(final String name) {
      return _programs.get(name);
   }


   private IGPUProgram compileProgramWithName(final GL gl,
                                              final String name) {
   ...
   private IGPUProgram getNewProgram(final GL gl,
                                     final int uniformsCode,
                                     final int attributesCode) {
      ...
      final boolean transformTC = (GPUVariable.hasUniform(uniformsCode, GPUUniformKey.TRANSLATION_TEXTURE_COORDS) || GPUVariable.hasUniform(
               uniformsCode, GPUUniformKey.SCALE_TEXTURE_COORDS));
   ...
   private IGPUProgram getCompiledProgram(final int uniformsCode,
                                          final int attributesCode) {
   ...
   public final IGPUProgram getProgram(final GL gl,
                                       final int uniformsCode,
                                       final int attributesCode) {
   ...
   public final void removeUnused() {
      final java.util.Iterator<java.util.Map.Entry<String, IGPUProgram>> iterator = _programs.entrySet().iterator();
      while (iterator.hasNext()) {
         final java.util.Map.Entry<String, IGPUProgram> entry = iterator.next();
         final IGPUProgram program = entry.getValue();
