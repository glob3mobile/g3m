...
public class FlatMercatorPlanet extends Planet
{
...
//#define RAD2DEG(a) ((a) * (180 / M_PI))
   private static final int EARTH_RADIUS = 6378137;
    ...
    private static double y2lat_m(double y)
    {
        return ((2 * IMathUtils.instance().atan(IMathUtils.instance().exp(y/EARTH_RADIUS)) - M_PI/2) * (180 / M_PI));
    }

    private static double x2lon_m(double x)
    {
        return ((x/EARTH_RADIUS) * (180 / M_PI));
    }

    private static double lat2y_m(double lat)
    {
        return IMathUtils.instance().log(IMathUtils.instance().tan(((lat) / (180 / M_PI)) / 2 + M_PI/4)) * EARTH_RADIUS;
    }

    private static double lon2x_m(double lon)
    {
        return ((lon) / (180 / M_PI)) * EARTH_RADIUS;
    }

    private static Vector2D getSize()
    {
        return new Vector2D(lat2y_m(85.0) - lat2y_m(-85.0), lon2x_m(180.0) - lon2x_m(-180.0));
    }
    ...
    public FlatMercatorPlanet()
    {
       _size = new Vector2D(getSize());
       _radii = new Vector3D(_size._x, _size._y, 0);
    ...
    {
        final double latDeg = latitude.clampedTo(Angle.fromDegrees(-85.0), Angle.fromDegrees(85.0))._degrees;
        Vector3D v = new Vector3D(lon2x_m(longitude._degrees), lat2y_m(latDeg), height);
        return v;
    }
    ...
    {
        final double x = lon2x_m(longitude._degrees);
        final double y = lat2y_m(latitude._degrees);
