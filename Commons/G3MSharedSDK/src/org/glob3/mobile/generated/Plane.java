...
public class Plane {
   ...
   public static Plane fromPoints(final Vector3D point0,
                                  final Vector3D point1,
                                  final Vector3D point2) {
   ...
   public final Plane transformedByTranspose(final MutableMatrix44D M) {
   ...
   public static Vector3D intersectionXYPlaneWithRay(final Vector3D origin,
                                                     final Vector3D direction) {
      if (direction._z == 0) {
         return Vector3D.nan();
      }
      ...
      final Vector3D point = origin.add(direction.times(t));
      return point;
   }


   public final boolean isVectorParallel(final Vector3D vector) {
      final double d = _normal.dot(vector);
      return ((d != d) || (IMathUtils.instance().abs(d) < 0.01));
   }


   public final Angle vectorRotationForAxis(final Vector3D vector,
                                            final Vector3D axis) {
      ...
      final double k4 = (-(a * b * d1_2 * x) + (c * d2_2 * (-(c * y) + (b * z))) + (a_2 * b * ((b * y) + (c * z))))
                        / (d1_2 * d2_2);
