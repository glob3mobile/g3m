...
//#endif
  ...
  private double convexPolygonArea()
  {
	double sum = 0;
  ...
  {
	java.util.ArrayList<Short> indices = calculateTrianglesIndexesByEarClipping();
	if (indices.size() % 3 != 0)
	{
  //C++ TO JAVA CONVERTER TODO TASK: The #define macro NAND was defined in alternate ways and cannot be replaced in-line:
	...
	{
	  Vector2D v1 = _coor2D.get(i);
	  Vector2D v2 = _coor2D.get(i+1);
	  Vector2D v3 = _coor2D.get(i+2);
  ...
  {
	final double area = convexPolygonArea();
	return area > 0;
  }
...
//#if C_CODE
	...
	{
	  if (_coor2D.get(j) != null)
		  _coor2D.get(j).dispose();
	}
//#endif
  }

  public final java.util.ArrayList<Short> calculateTrianglesIndexesByEarClipping()
  {
	...
	java.util.ArrayList<Short> finalListOfIndexes = new java.util.ArrayList<Short>();
	...
	while (remainingCorners.size() > 2)
	{
	  ...
	  {
		finalListOfIndexes.add(remainingIndexes.get(i1));
		finalListOfIndexes.add(remainingIndexes.get(i2));
		finalListOfIndexes.add(remainingIndexes.get(i3));
  
		//Removing ear
//C++ TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
//#if C_CODE
//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent to the STL vector 'erase' method in Java:
		remainingCorners.erase(remainingCorners.iterator() + i2);
//C++ TO JAVA CONVERTER TODO TASK: There is no direct equivalent to the STL vector 'erase' method in Java:
		remainingIndexes.erase(remainingIndexes.iterator() + i2);
//#endif
//C++ TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
//#if JAVA_CODE
