...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: PlanetTileTessellatorData* getTessellatorData() const
...
//ORIGINAL LINE: const Tile* subtileResult = subtile->getDeepestTileContaining(position);
		final Tile subtileResult = subtile.getDeepestTileContaining(new Geodetic3D(position));
...
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: if (renderedSector == null || renderedSector->touchesWith(s1))
	if (renderedSector == null || renderedSector.touchesWith(new Sector(s1)))
	{
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: subTiles->push_back(createSubTile(lower._latitude, lower._longitude, splitLatitude, splitLongitude, nextLevel, row2, column2, setParent));
	  subTiles.add(createSubTile(lower._latitude, lower._longitude, new Angle(splitLatitude), new Angle(splitLongitude), nextLevel, row2, column2, setParent));
	}
  
	Sector s2 = new Sector(new Geodetic2D(lower._latitude, splitLongitude), new Geodetic2D(splitLatitude, upper._longitude));
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: if (renderedSector == null || renderedSector->touchesWith(s2))
	if (renderedSector == null || renderedSector.touchesWith(new Sector(s2)))
	{
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: subTiles->push_back(createSubTile(lower._latitude, splitLongitude, splitLatitude, upper._longitude, nextLevel, row2, column2 + 1, setParent));
	  subTiles.add(createSubTile(lower._latitude, new Angle(splitLongitude), new Angle(splitLatitude), upper._longitude, nextLevel, row2, column2 + 1, setParent));
	}
  
	Sector s3 = new Sector(new Geodetic2D(splitLatitude, lower._longitude), new Geodetic2D(upper._latitude, splitLongitude));
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: if (renderedSector == null || renderedSector->touchesWith(s3))
	if (renderedSector == null || renderedSector.touchesWith(new Sector(s3)))
	{
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: subTiles->push_back(createSubTile(splitLatitude, lower._longitude, upper._latitude, splitLongitude, nextLevel, row2 + 1, column2, setParent));
	  subTiles.add(createSubTile(new Angle(splitLatitude), lower._longitude, upper._latitude, new Angle(splitLongitude), nextLevel, row2 + 1, column2, setParent));
	}
  
	Sector s4 = new Sector(new Geodetic2D(splitLatitude, splitLongitude), new Geodetic2D(upper._latitude, upper._longitude));
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: if (renderedSector == null || renderedSector->touchesWith(s4))
	if (renderedSector == null || renderedSector.touchesWith(new Sector(s4)))
	{
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: subTiles->push_back(createSubTile(splitLatitude, splitLongitude, upper._latitude, upper._longitude, nextLevel, row2 + 1, column2 + 1, setParent));
	  subTiles.add(createSubTile(new Angle(splitLatitude), new Angle(splitLongitude), upper._latitude, upper._longitude, nextLevel, row2 + 1, column2 + 1, setParent));
	}
...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: boolean isElevationDataSolved() const
...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: ElevationData* getElevationData() const
...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: ElevationData* createElevationDataSubviewFromAncestor(Tile* ancestor) const
...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: Vector2I getNormalizedPixelFromPosition(const Geodetic2D& position, const Vector2I& tileDimension) const
...
//C++ TO JAVA CONVERTER WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: const Vector2D uv = _sector.getUVCoordinates(position);
	final Vector2D uv = _sector.getUVCoordinates(new Geodetic2D(position));
	return new Vector2I(math.toInt(tileDimension._x * uv._x), math.toInt(tileDimension._y * uv._y));
  }

//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: TileData* getData(int id) const
...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: void setData(TileData* data) const
...
//#if C_CODE
		...
		for (int i = oldDataSize; i < _dataSize; i++)
		{
		  _data[i] = null;
		}
		oldData = null;
...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: void clearDataWithID(int id) const
...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: const TileTessellatorMeshData* getTessellatorMeshData() const
  ...
  public final Mesh getTessellatorMesh(G3MRenderContext rc, PlanetRenderContext prc)
  {
	...
	if ((_tessellatorMesh == null) || _mustActualizeMeshDueToNewElevationData)
	{
	  ...
	  if (elevationDataProvider == null)
	  {
		...
		tangible.RefObject<TileTessellatorMeshData> tempRef__tileTessellatorMeshData = new tangible.RefObject<TileTessellatorMeshData>(_tileTessellatorMeshData);
		_tessellatorMesh = prc._tessellator.createTileMesh(rc, prc, this, null, tempRef__tileTessellatorMeshData);
		_tileTessellatorMeshData = tempRef__tileTessellatorMeshData.argvalue;
	  }
	  else
	  {
		tangible.RefObject<TileTessellatorMeshData> tempRef__tileTessellatorMeshData2 = new tangible.RefObject<TileTessellatorMeshData>(_tileTessellatorMeshData);
		Mesh tessellatorMesh = prc._tessellator.createTileMesh(rc, prc, this, _elevationData, tempRef__tileTessellatorMeshData2);
		_tileTessellatorMeshData = tempRef__tileTessellatorMeshData2.argvalue;
...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: boolean hasSubtiles() const
