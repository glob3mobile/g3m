...
public class Tile {
   ...
   private final TileTessellatorMeshData _tileTessellatorMeshData = new TileTessellatorMeshData();
   ...
   private void computeTileCorners(final Planet planet) {
   ...
   private void prepareTestLODData(final Planet planet) {
      ...
      final Angle latitudeAngle = nN.angleBetween(nS);
      final double latRad = latitudeAngle._radians;
      final double sin_lat_2 = java.lang.Math.sin(latRad / 2);
      final double latitudeArcSegmentRatio = sin_lat_2 == 0 ? 1 : latRad / (2 * sin_lat_2);

      final Angle longitudeAngle = nE.angleBetween(nW);
   ...
   private Mesh getTessellatorMesh(final G3MRenderContext rc,
                                   final ElevationDataProvider elevationDataProvider,
                                   final TileTessellator tessellator,
      ...
      if ((_elevationData == null) && (elevationDataProvider != null) && (elevationDataProvider.isEnabled())) {
         initializeElevationData(elevationDataProvider, tessellator, layerTilesRenderParameters._tileMeshResolution,
                  rc.getPlanet(), tilesRenderParameters._renderDebug);
      }
      ...
      if ((_tessellatorMesh == null) || _mustActualizeMeshDueToNewElevationData) {
         ...
         if (elevationDataProvider == null) {
            ...
            _tessellatorMesh = tessellator.createTileMesh(rc.getPlanet(), layerTilesRenderParameters._tileMeshResolution, this,
                     null, _verticalExaggeration, layerTilesRenderParameters._mercator, tilesRenderParameters._renderDebug,
         ...
         _middleNorthPoint = new Vector3D(rc.getPlanet().toCartesian(gN));
         _middleSouthPoint = new Vector3D(rc.getPlanet().toCartesian(gS));
         _middleEastPoint = new Vector3D(rc.getPlanet().toCartesian(gE));
         _middleWestPoint = new Vector3D(rc.getPlanet().toCartesian(gW));
      }
   ...
   private Mesh getDebugMesh(final G3MRenderContext rc,
                             final TileTessellator tessellator,
                             final LayerTilesRenderParameters layerTilesRenderParameters) {
      if (_debugMesh == null) {
   ...
   private boolean isVisible(final G3MRenderContext rc,
                             final Planet planet,
                             final Vector3D cameraNormalizedPosition,
                             final double cameraAngle2HorizonInRadians,
                             final Frustum cameraFrustumInModelCoordinates,
                             final ElevationDataProvider elevationDataProvider,
      ...
      final BoundingVolume boundingVolume = getBoundingVolume(rc, elevationDataProvider, tessellator, layerTilesRenderParameters,
   ...
   private boolean meetsRenderCriteria(final G3MRenderContext rc,
                                       final LayerTilesRenderParameters layerTilesRenderParameters,
                                       final TileTexturizer texturizer,
                                       final TilesRenderParameters tilesRenderParameters,
                                       final TilesStatistics tilesStatistics,
                                       final ITimer lastSplitTimer,
                                       final double texWidthSquared,
                                       final double texHeightSquared,
                                       final double nowInMS) {
   ...
   private void rawRender(final G3MRenderContext rc,
                          ...
                          final TilesRenderParameters tilesRenderParameters,
                          final boolean isForcedFullRender,
                          final long texturePriority,
                          final boolean logTilesPetitions) {

      final Mesh tessellatorMesh = getTessellatorMesh(rc, elevationDataProvider, tessellator, layerTilesRenderParameters,
               tilesRenderParameters);
      ...
      if (texturizer == null) {
         ...
         if (needsToCallTexturizer) {
            _texturizedMesh = texturizer.texturize(rc, tessellator, tileRasterizer, layerTilesRenderParameters, layerSet,
                     isForcedFullRender, texturePriority, this, tessellatorMesh, _texturizedMesh, logTilesPetitions);
         }
         ...
         if (_texturizedMesh != null) {
            ...
            if (_flatColorMesh == null) {
               _flatColorMesh = new FlatColorMesh(tessellatorMesh, false, Color.newFromRGBA((float) 1.0, (float) 1.0,
   ...
   private void debugRender(final G3MRenderContext rc,
                            final GLState glState,
                            final TileTessellator tessellator,
                            final LayerTilesRenderParameters layerTilesRenderParameters) {
      final Mesh debugMesh = getDebugMesh(rc, tessellator, layerTilesRenderParameters);
      if (debugMesh != null) {
         //debugMesh->render(rc);
   ...
   private Tile createSubTile(final Angle lowerLat,
      ...
      final Tile parent = setParent ? this : null;
      return new Tile(_texturizer, parent, new Sector(new Geodetic2D(lowerLat, lowerLon), new Geodetic2D(upperLat, upperLon)),
               level, row, column, _planetRenderer);
   }


   private java.util.ArrayList<Tile> getSubTiles(final Angle splitLatitude,
   ...
   private void deleteTexturizedMesh(final TileTexturizer texturizer) {
   ...
   private BoundingVolume getBoundingVolume(final G3MRenderContext rc,
                                            final ElevationDataProvider elevationDataProvider,
                                            final TileTessellator tessellator,
                                            final LayerTilesRenderParameters layerTilesRenderParameters,
                                            final TilesRenderParameters tilesRenderParameters) {
      if (_boundingVolume == null) {
         final Mesh mesh = getTessellatorMesh(rc, elevationDataProvider, tessellator, layerTilesRenderParameters,
                  tilesRenderParameters);
         if (mesh != null) {
   ...
   public final void prepareForFullRendering(final G3MRenderContext rc,
                                             final TileTexturizer texturizer,
                                             final ElevationDataProvider elevationDataProvider,
                                             final TileTessellator tessellator,
                                             final TileRasterizer tileRasterizer,
   ...
   public final void render(final G3MRenderContext rc,
      ...
      if (isVisible(rc, planet, cameraNormalizedPosition, cameraAngle2HorizonInRadians, cameraFrustumInModelCoordinates,
               elevationDataProvider, renderedSector, tessellator, layerTilesRenderParameters, tilesRenderParameters)) {
         setIsVisible(true, texturizer);

         tilesStatistics.computeVisibleTile(this);

         final boolean isRawRender = ((toVisitInNextIteration == null)
                                      || meetsRenderCriteria(rc, layerTilesRenderParameters, texturizer, tilesRenderParameters,
                                               tilesStatistics, lastSplitTimer, texWidthSquared, texHeightSquared, nowInMS) || (tilesRenderParameters._incrementalTileQuality && !_textureSolved));

         if (isRawRender) {
            if (renderTileMeshes) {
               rawRender(rc, parentState, texturizer, elevationDataProvider, tessellator, tileRasterizer,
                        layerTilesRenderParameters, layerSet, tilesRenderParameters, isForcedFullRender, texturePriority,
                        logTilesPetitions);
            }
            ...
            final Angle splitLatitude = layerTilesRenderParameters._mercator ? MercatorUtils.calculateSplitLatitude(
                     lower._latitude, upper._latitude) : Angle.midAngle(lower._latitude, upper._latitude);
            /*                               */
            /*                               */

            final java.util.ArrayList<Tile> subTiles = getSubTiles(splitLatitude, splitLongitude);
   ...
   public final java.util.ArrayList<Tile> getSubTiles(final boolean mercator) {
      final Geodetic2D lower = _sector._lower;
      final Geodetic2D upper = _sector._upper;
   ...
   public final void prepareForFullRendering(final G3MRenderContext rc,
      ...
      if (texturizer != null) {
         ...
         if (needsToCallTexturizer) {
            _texturizedMesh = texturizer.texturize(rc, tessellator, tileRasterizer, layerTilesRenderParameters, layerSet,
                     isForcedFullRender, texturePriority, this, tessellatorMesh, _texturizedMesh, false);
         }
      }
   }
   ...
   public final void render(final G3MRenderContext rc,
      ...
      if (isVisible(rc, planet, cameraNormalizedPosition, cameraAngle2HorizonInRadians, cameraFrustumInModelCoordinates,
         ...
         if (isRawRender) {
            if (renderTileMeshes) {
               rawRender(rc, parentState, texturizer, elevationDataProvider, tessellator, tileRasterizer,
                        layerTilesRenderParameters, layerSet, tilesRenderParameters, isForcedFullRender, texturePriority, false);
            }
            if (tilesRenderParameters._renderDebug) {
               debugRender(rc, parentState, tessellator, layerTilesRenderParameters);
            }
   ...
   public final void setTessellatorData(final PlanetTileTessellatorData tessellatorData) {
   ...
   public final void prune(final TileTexturizer texturizer,
                           final ElevationDataProvider elevationDataProvider) {
      if (_subtiles != null) {
   ...
   public final void toBeDeleted(final TileTexturizer texturizer,
                                 final ElevationDataProvider elevationDataProvider) {
      if (texturizer != null) {
   ...
   public final java.util.ArrayList<Tile> createSubTiles(final Angle splitLatitude,
                                                         final Angle splitLongitude,
                                                         final boolean setParent) {
      final Geodetic2D lower = _sector._lower;
      final Geodetic2D upper = _sector._upper;
      ...
      final Sector s1 = new Sector(new Geodetic2D(lower._latitude, lower._longitude), new Geodetic2D(splitLatitude,
               splitLongitude));
      ...
      final Sector s2 = new Sector(new Geodetic2D(lower._latitude, splitLongitude), new Geodetic2D(splitLatitude,
               upper._longitude));
      ...
      final Sector s3 = new Sector(new Geodetic2D(splitLatitude, lower._longitude), new Geodetic2D(upper._latitude,
               splitLongitude));
      ...
      final Sector s4 = new Sector(new Geodetic2D(splitLatitude, splitLongitude), new Geodetic2D(upper._latitude,
               upper._longitude));
   ...
   public final void setElevationData(final ElevationData ed,
                                      final int level) {
   ...
   public final void getElevationDataFromAncestor(final Vector2I extent) {
      if (_elevationData == null) {
         ...
         if (ancestor != null) {
            final ElevationData subView = createElevationDataSubviewFromAncestor(ancestor);
   ...
   public final void initializeElevationData(final ElevationDataProvider elevationDataProvider,
                                             final TileTessellator tessellator,
                                             final Vector2I tileMeshResolution,
                                             final Planet planet,
   ...
   public final void ancestorChangedElevationData(final Tile ancestor) {
   ...
   public final ElevationData createElevationDataSubviewFromAncestor(final Tile ancestor) {
      final ElevationData ed = ancestor.getElevationData();
   ...
   public final Vector2I getPixelNormalizedFromPosition(final Geodetic2D position2D,
                                                        final Vector2I size) {
