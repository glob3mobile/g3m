...
public class Tile {
   ...
   private Mesh getTessellatorMesh(final G3MRenderContext rc,
      ...
      final ElevationDataProvider elevationDataProvider = prc.getElevationDataProvider();
      ...
      if ((_elevationData == null) && (elevationDataProvider != null)) {
         initializeElevationData(elevationDataProvider, prc.getTessellator(),
                  prc.getLayerTilesRenderParameters()._tileMeshResolution, rc.getPlanet(), prc.getParameters()._renderDebug);
      }
      ...
      if ((_tessellatorMesh == null) || _mustActualizeMeshDueToNewElevationData) {
         ...
         if (elevationDataProvider == null) {
            ...
            _tessellatorMesh = prc.getTessellator().createTileMesh(rc.getPlanet(),
                     layerTilesRenderParameters._tileMeshResolution, this, null, _verticalExaggeration,
                     layerTilesRenderParameters._mercator, prc.getParameters()._renderDebug);

         }
         else {
            final Mesh tessellatorMesh = prc.getTessellator().createTileMesh(rc.getPlanet(),
   ...
   private boolean meetsRenderCriteria(final G3MRenderContext rc,
      ...
      final TileTexturizer texturizer = prc.getTexturizer();
      if (texturizer != null) {
   ...
   private void rawRender(final G3MRenderContext rc,
      ...
      final Mesh tessellatorMesh = getTessellatorMesh(rc, prc);
   ...
   private void debugRender(final G3MRenderContext rc,
                            final PlanetRendererContext prc,
                            final GLState glState) {
      final Mesh debugMesh = getDebugMesh(rc, prc);
      if (debugMesh != null) {
         //debugMesh->render(rc);
   ...
   private Box getTileBoundingVolume(final G3MRenderContext rc) {
   ...
   private BoundingVolume getBoundingVolume(final G3MRenderContext rc,
      ...
      if (_boundingVolume == null) {
         final Mesh mesh = getTessellatorMesh(rc, prc);
         if (mesh != null) {
   ...
   public final void render(final G3MRenderContext rc,
      ...
      final TilesStatistics statistics = prc.getStatistics();
      ...
      if (isVisible(rc, prc, planet, cameraNormalizedPosition, cameraAngle2HorizonInRadians, cameraFrustumInModelCoordinates)) {
         ...
         if (isRawRender) {
            ...
            final Angle splitLatitude = prc.getLayerTilesRenderParameters()._mercator ? MercatorUtils.calculateSplitLatitude(
   ...
   public final java.util.ArrayList<Tile> createSubTiles(final Angle splitLatitude,
      ...
      subTiles.add(createSubTile(lower._latitude, splitLongitude, splitLatitude, upper._longitude, nextLevel, row2, column2 + 1,
               setParent));

      subTiles.add(createSubTile(splitLatitude, lower._longitude, upper._latitude, splitLongitude, nextLevel, row2 + 1, column2,
               setParent));
   ...
   public final ElevationData createElevationDataSubviewFromAncestor(final Tile ancestor) {
      ...
      if ((_lastElevationDataProvider != null) && (_lastTileMeshResolutionX > 0) && (_lastTileMeshResolutionY > 0)) {
         return new DecimatedSubviewElevationData(ed, getSector(), new Vector2I(_lastTileMeshResolutionX,
                  _lastTileMeshResolutionY));
      }
