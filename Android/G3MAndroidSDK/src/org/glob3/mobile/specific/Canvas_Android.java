...
public class Canvas_Android extends ICanvas {
	...
	final private RectF _rect = new RectF(); // RectF instance for reuse (and
	// avoid garbage)
	...
	private static int toAndroidColor(
			final org.glob3.mobile.generated.Color g3mColor) {
	...
	@Override
	protected void _setShadow(final Color color, final float blur,
			final float offsetX, final float offsetY) {
		_fillPaint
		.setShadowLayer(blur, offsetX, offsetY, toAndroidColor(color));
		_strokePaint.setShadowLayer(blur, offsetX, offsetY,
				toAndroidColor(color));
	}
	...
	@Override
	protected void _fillRectangle(final float left, final float top,
			final float width, final float height) {
		_canvas.drawRect(left, top, left + width, top + height, _fillPaint);
	}

	@Override
	protected void _strokeRectangle(final float left, final float top,
			final float width, final float height) {
		_canvas.drawRect(left, top, left + width, top + height, _strokePaint);
	}

	@Override
	protected void _fillAndStrokeRectangle(final float left, final float top,
			final float width, final float height) {
	...
	@Override
	protected void _fillRoundedRectangle(final float left, final float top,
	...
	@Override
	protected void _strokeRoundedRectangle(final float left, final float top,
			final float width, final float height, final float radius) {
		_rect.set(left, top, left + width, top + height);
	...
	protected void _fillAndStrokeRoundedRectangle(final float left,
			final float top, final float width, final float height,
			final float radius) {
		_rect.set(left, top, left + width, top + height);
		_canvas.drawRoundRect(_rect, radius, radius, _fillPaint);
		_canvas.drawRoundRect(_rect, radius, radius, _strokePaint);
	...
	@Override
	protected void _fillText(final String text, final float left,
			final float top) {
		_canvas.drawText(text, left, top, _fillPaint);
	}

	@Override
	protected void _drawImage(IImage image, float left, float top) {
		Bitmap bitmap = ((Image_Android) image).getBitmap();
		_canvas.drawBitmap(bitmap, left, top, null);
	}

	@Override
	protected void _drawImage(IImage image, float left, float top, float width,
			float height) {
		Bitmap bitmap = ((Image_Android) image).getBitmap();
