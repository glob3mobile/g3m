...
public final class NativeGL2_Android extends INativeGL {
		...
		if (_openGLThread != null) {
			...
			if (currentThread != _openGLThread) {
				throw new RuntimeException(
						"OpenGL code executed from a Non-OpenGL thread.  (OpenGLThread="
								+ _openGLThread + ", CurrentThread="
								+ currentThread + ")");
			}
		}
	}
	...
	// checkOpenGLThread();
	// GLES20.glUniformMatrix4fv(((GLUniformID_Android) location).getID(), 1,
	// transpose,
	...
	@Override
	public void clearColor(final float red, final float green,
	...
	@Override
	public void uniform4f(final IGLUniformID location, final float v0,
	...
	public int getError() {
		...
		if (e != GLES20.GL_NO_ERROR) {
			if (e == GLES20.GL_INVALID_ENUM) {
				ILogger.instance().logError("GLES20: INVALID_ENUM");
			}

			if (e == GLES20.GL_INVALID_VALUE) {
				ILogger.instance().logError("GLES20: INVALID_VALUE");
			}

			if (e == GLES20.GL_INVALID_OPERATION) {
				ILogger.instance().logError("GLES20: INVALID_OPERATION");
			}

			if (e == GLES20.GL_OUT_OF_MEMORY) {
				ILogger.instance().logError("GLES20: GL_OUT_OF_MEMORY");
			}
		}
		return e;
	}
	...
	@Override
	public void bindTexture(final int target, final IGLTextureId texture) {
		checkOpenGLThread();
	...
	public ArrayList<IGLTextureId> genTextures(final int count) {
		checkOpenGLThread();
		final ArrayList<IGLTextureId> result = new ArrayList<IGLTextureId>(
				count);
	...
	@Override
	public void vertexAttribPointer(final int index, final int size,
		...
		// final FloatBuffer floatBuffer = ((FloatBuffer_Android)
		// buffer).getBuffer();
		// GLES20.glVertexAttribPointer(index, size, GLES20.GL_FLOAT,
	...
	@Override
	public void drawElements(final int mode, final int count,
			final IShortBuffer indices) {
		checkOpenGLThread();

		final ShortBuffer indexBuffer = ((ShortBuffer_Android) indices)
				.getBuffer();
	...
	@Override
	public void texImage2D(final IImage image, final int format) {
		checkOpenGLThread();
	...
	@Override
	public void bindAttribLocation(final GPUProgram program, final int loc,
			final String name) {
		checkOpenGLThread();
	...
	@Override
	public int getProgramiv(final GPUProgram program, final int param) {
		checkOpenGLThread();
	...
	@Override
	public GPUUniform getActiveUniform(final GPUProgram program, final int i) {
		checkOpenGLThread();
		final int[] maxLength = new int[1];
		GLES20.glGetProgramiv(program.getProgramID(),
				GLES20.GL_ACTIVE_UNIFORM_MAX_LENGTH, maxLength, 0);
		...
		GLES20.glGetActiveUniform(program.getProgramID(), i, bufsize, length,
		...
		case GLES20.GL_FLOAT_MAT4:
			return new GPUUniformMatrix4Float(nameStr, new GLUniformID_Android(
					id));
	...
	@Override
	public GPUAttribute getActiveAttribute(final GPUProgram program, final int i) {
		checkOpenGLThread();
		final int[] maxLength = new int[1];
		GLES20.glGetProgramiv(program.getProgramID(),
				GLES20.GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, maxLength, 0);
		...
		GLES20.glGetActiveAttrib(program.getProgramID(), i, bufsize, length, 0,
				size, 0, type, 0, name, 0);

		try {
			String nameStr = new String(name, "UTF-8");
			nameStr = nameStr.substring(0, length[0]);
			final int id = GLES20.glGetAttribLocation(program.getProgramID(),
					nameStr);
	...
	@Override
	public void uniform3f(final IGLUniformID location, final float v0,
