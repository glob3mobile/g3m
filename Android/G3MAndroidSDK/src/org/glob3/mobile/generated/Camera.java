 ...
 */
public class Camera {
   public Camera(final Camera that) {
      _width = that._width;
      ...
      _frustumInModelCoordinates = (that._frustumInModelCoordinates == null) ? null
                                                                            : new Frustum(that._frustumInModelCoordinates);
      _halfFrustum = (that._halfFrustum == null) ? null : new Frustum(that._halfFrustum);
      _halfFrustumInModelCoordinates = (that._halfFrustumInModelCoordinates == null) ? null : new Frustum(
               that._halfFrustumInModelCoordinates);
   ...
   public final void copyFrom(final Camera that) {
      _width = that._width;
      _height = that._height;
      ...
      _halfFrustumInModelCoordinates = (that._frustumInModelCoordinates == null) ? null : new Frustum(
               that._frustumInModelCoordinates);
   }
   ...
   public final void render(final RenderContext rc) {
      ...
      if (false) {
         ...
         final Vector3D p0 = new Vector3D(new Vector3D(data._left / 4, data._top / 4, -data._znear - 10).transformedBy(inversed,
                  1));
         final Vector3D p1 = new Vector3D(new Vector3D(data._left / 4, data._bottom / 4, -data._znear - 10).transformedBy(
                  inversed, 1));
         final Vector3D p2 = new Vector3D(new Vector3D(data._right / 4, data._bottom / 4, -data._znear - 10).transformedBy(
                  inversed, 1));
         final Vector3D p3 = new Vector3D(new Vector3D(data._right / 4, data._top / 4, -data._znear - 10).transformedBy(inversed,
                  1));

         final float[] v = { (float) p0._x, (float) p0._y, (float) p0._z, (float) p1._x, (float) p1._y, (float) p1._z,
                  (float) p2._x, (float) p2._y, (float) p2._z, (float) p3._x, (float) p3._y, (float) p3._z };
         final int[] i = { 0, 1, 2, 3 };

         final FloatBufferBuilderFromCartesian3D vertices = new FloatBufferBuilderFromCartesian3D(CenterStrategy.noCenter(),
                  Vector3D.zero());
         final IntBufferBuilder index = new IntBufferBuilder();
         ...
         final IIntBuffer _indices = index.create();
         final IFloatBuffer _vertices = vertices.create();
   ...
   public final Vector3D pixel2Ray(final Vector2I pixel) {
   ...
   public final Vector3D pixel2PlanetPoint(final Vector2I pixel) {
   ...
   public final Vector2I point2Pixel(final Vector3D point) {
   ...
   public final void moveForward(final double d) {
   ...
   public final void setCartesianPosition(final MutableVector3D v) {
   ...
   private static class CameraEffectTarget
            implements
               EffectTarget {
      @Override
   ...
   private void setCenter(final MutableVector3D v) {
   ...
   private void setUp(final MutableVector3D v) {
      if (!v.equalTo(_up)) {
   ...
   private Frustum getHalfFrustum() {
      if (_dirtyFlags._halfFrustum) {
         ...
         final FrustumData data = getFrustumData();
