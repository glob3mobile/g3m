...
public class OSMLayer extends Layer
{
  ...
  public OSMLayer(URL mapServerURL, LayerCondition condition, Sector sector) //http://a.tile.openstreetmap.org/level/x/y.png
  {
	  super(condition);
  ...
  public final java.util.ArrayList<Petition> getMapPetitions(RenderContext rc, Tile tile, int width, int height)
  {
	...
	for(int x = lowerTileXY[0]; x<= lowerTileXY[0]+deltaX; x++)
	{
	  for(int y = upperTileXY[1]; y<=upperTileXY[1]+deltaY; y++)
	  {
		...
		strs <<".png";
		String url = strs.str();
		Petition petition = new Petition(osmSector, new URL(url));
...
//C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in Java:
//ORIGINAL LINE: Sector getOSMTileAsSector(const int tileXY[], const int level)const
  public final Sector getOSMTileAsSector(int[] tileXY, int level)
  {
