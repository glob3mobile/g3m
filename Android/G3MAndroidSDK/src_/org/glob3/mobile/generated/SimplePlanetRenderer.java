...
//ORIGINAL LINE: float* createTextureCoordinates()
  ...
  private boolean initializeMesh(RenderContext rc)
  {
	...
	if (true)
	{
	  ...
	  {
		float val = 0.5 + sinf(2.0 * Math.PI * ((float) i) / numVertices) / 2.0;
	...
	_mesh = new IndexedMesh(true, GLPrimitive.TriangleStrip, ver, ind, numIndexes, flatColor, colors, texID, texC, normals);
