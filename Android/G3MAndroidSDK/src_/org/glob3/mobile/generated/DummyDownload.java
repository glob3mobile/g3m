...
public class DummyDownload implements IDownloadListener
{
  ...
  {
	String url = "http://demo.cubewerx.com/demo/cubeserv/cubeserv.cgi?Request=GetCapabilities&SERVICE=WMS";
	String url2 = "http://ovc.catastro.meh.es/Cartografia/WMS/ServidorWMS.aspx?request=capabilities";
	...
	public final void runSqlite(String root, String filename, IStorage fssAux)
	{
		...
		{
		  ByteBuffer bb = _fss.getByteBuffer(filename);
		  String resp = (String)bb.getData();
		  System.out.printf("\nFileName: %s;\nData: %s;\nDataLength:%i;\n\n",(root+filename), resp, bb.getDataLength());
