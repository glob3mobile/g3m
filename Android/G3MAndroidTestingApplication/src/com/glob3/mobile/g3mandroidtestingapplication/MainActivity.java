...
public class MainActivity extends Activity {
	...
	private G3MWidget_Android createWidgetWithYouAreHere() {
		...
		class YouAreHereTask extends GTask {
			...
			public void run(G3MContext context) {
				...
				if (_mark == null) {
					...
					_mark = new Mark(new URL(URL.FILE_PROTOCOL
							+ "g3m-marker.png"), Geodetic3D.fromDegrees(
	...
	private G3MWidget_Android createWidget() {
		...
		renderer.setTouchListener(new NonOverlappingMarkTouchListener() {
			...
			public boolean touchedMark(final NonOverlappingMark mark,
				...
				System.out.println("Touched on pixel=" + touchedPixel
						+ ", mark=" + mark);
				return true;
			}
		});
				...
				public void run(final G3MContext context) {
					...
					final IImageDownloadListener listener = new IImageDownloadListener() {
						...
						public void onDownload(final URL url,
							...
							// canvas.drawImage(image, width / 4, height / 4);
							// // ok

							canvas.drawImage(image, width / 8, height / 8); // ok
							canvas.drawImage(image, (width / 8) * 3,
									height / 8, 0.5f); // ok

							canvas.drawImage(image, width / 8,
									(height / 8) * 3, image.getWidth() * 2,
									image.getHeight() * 2); // ok
							canvas.drawImage(image, (width / 8) * 3,
									(height / 8) * 3, image.getWidth() * 2,
									image.getHeight() * 2, 0.5f); // ok
