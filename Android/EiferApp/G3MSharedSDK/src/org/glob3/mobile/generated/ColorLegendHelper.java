...
//#warning TODO CHECK CJenksBreaks
  ...
  public static ColorLegend createColorBrewLegendWithHomogeneousBreaks(java.util.ArrayList<Double> sListDouble,
                                                                       final String colorName,
                                                                             int sClassCount)
  {
    java.util.ArrayList<Double> naturalBreaks = new ArrayList<>();

    java.util.Collections.sort(sListDouble); //TODO revisar


    final double size = sListDouble.size();
    final double q1 = sListDouble.get((int) IMathUtils.instance().floor( size * 0.25 ));
    final double q3 = sListDouble.get((int) IMathUtils.instance().floor( size * 0.75 ));
    final double iqr = q3-q1;

    final double begin = q1 - 1.5 * iqr > sListDouble.get(0)? q1 - 1.5 * iqr : sListDouble.get(0);
    final double end = q3 + 1.5 * iqr < sListDouble.get(sListDouble.size()-1)? q3 + 1.5 * iqr : sListDouble.get(sListDouble.size()-1);
...
//  const double end = sListDouble.at((size_t)IMathUtils::instance()->floor( size * 0.8 )); //Removing outliers
    ...
    if (naturalBreaks.size() != colorNames.size()){
      throw new IllegalArgumentException("Error at ColorLegendHelper::createColorBrewLegendWithNaturalBreaks()");
    }

    java.util.ArrayList<ColorLegend.ColorAndValue> legend = new ArrayList<>();
    for (int i = 0; i < sClassCount;i++) {
      Color c = Color.parse(colorNames.get(i));
      legend.add(new ColorLegend.ColorAndValue(c, naturalBreaks.get(i)));
      c = null;
