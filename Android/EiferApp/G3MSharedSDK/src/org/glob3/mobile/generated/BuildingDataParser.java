...
public class BuildingDataParser
{
  ...
  private static int extractFeature(CityGMLBuilding b, String name, String nameInJSON, String data, int pos)
  {
    ...
    if (heatDem._endingPos == data.length()-1)
    {
      return heatDem._endingPos;
    }
    pos = heatDem._endingPos+1;
    // Chano working on this.
    //String [] sArray =  heatDem._string.split("[0-9]*\\.?[0-9]*");
    String [] sArray =  heatDem._string.split(" ");
    double v = IStringUtils.instance().parseDouble(sArray[0]);
	b.addNumericProperty(new CityGMLBuildingNumericProperty(name, v));
    /*if (sArray.length > 0) {
    	for (int i=0; i<sArray.length; i++){
    		if (!sArray[i].contentEquals("")) {
    			double v = IStringUtils.instance().parseDouble(sArray[i]);
    			b.addNumericProperty(new CityGMLBuildingNumericProperty(name, v));
    		}
    	}
    }*/
  ...
  public static Mesh createPointCloudMesh(String data, Planet planet, ElevationData elevationData)
  {
    ...
    DirectMesh dm = new DirectMesh(GLPrimitive.points(), true, vertices.getCenter(), vertices.create(), 1.0f, 2.0f, null, colors.create()); //new Color(Color::red()),
  ...
  //    h = h + scale * (h - minH);
    ...
    PointCloudMesh pcm = new PointCloudMesh(true, vertices.getCenter(), vertices.create(), 10.0f, colors, true, Color.blue());
