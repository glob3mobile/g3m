...
public:
	...
	SQResultSet (const SQDatabase* db, std::string* query, std::vector<void*> args);
	~SQResultSet();

	static SQResultSet* initWithDatabase(const SQDatabase* db, std::string* query, std::vector<void*> args);
	...
	virtual const void* dataColumnByIndex(int index) const;
...
public:
	...
	virtual SQResultSet* executeQuery(std::string* sql, std::vector<void*> args) const;
	...
	virtual void bindObjectAtColumnToStatement(void* obj, int idx, sqlite3_stmt* stmt) const;
	virtual bool executeNonQuery(std::string* sql, ...) const;
	virtual bool executeNonQuery(std::string* sql, std::vector<void*> args) const;
