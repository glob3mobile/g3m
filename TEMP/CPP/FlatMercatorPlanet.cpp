...
#include "FlatMercatorPlanet.hpp"
#include "Plane.hpp"
#include "CameraEffects.hpp"
#include "Camera.hpp"


const Planet* FlatMercatorPlanet::createEarth() {
  return new FlatMercatorPlanet();
}

FlatMercatorPlanet::FlatMercatorPlanet():
_size(getSize()),
_radii(_size._x, _size._y, 0)
{
    
}

Vector3D FlatMercatorPlanet::geodeticSurfaceNormal(const Angle& latitude,
                                           const Angle& longitude) const {
  return Vector3D(0, 0, 1);
}

std::vector<double> FlatMercatorPlanet::intersectionsDistances(double originX,
                                                       double originY,
                                                       double originZ,
                                                       double directionX,
                                                       double directionY,
...
Geodetic2D FlatMercatorPlanet::toGeodetic2D(const Vector3D& position) const {
...
//  return (Geodetic2D(Angle::fromDegrees(latitude), Angle::fromDegrees(longitude)));
    ...
    return Geodetic2D::fromDegrees(y2lat_m(position._y),x2lon_m(position._x));
}

Geodetic3D FlatMercatorPlanet::toGeodetic3D(const Vector3D& position) const {
  return Geodetic3D(toGeodetic2D(position), position._z);
}

Vector3D FlatMercatorPlanet::scaleToGeodeticSurface(const Vector3D& position) const {
  return Vector3D(position._x, position._y, 0);
}


Vector3D FlatMercatorPlanet::scaleToGeocentricSurface(const Vector3D& position) const {
  return scaleToGeodeticSurface(position);
}

Geodetic2D FlatMercatorPlanet::getMidPoint (const Geodetic2D& P0, const Geodetic2D& P1) const {
...
double FlatMercatorPlanet::computePreciseLatLonDistance(const Geodetic2D& g1,
                                                const Geodetic2D& g2) const {
  return toCartesian(g1).sub(toCartesian(g2)).length();
}

// compute distance from two points
double FlatMercatorPlanet::computeFastLatLonDistance(const Geodetic2D& g1,
                                             const Geodetic2D& g2) const {
  return computePreciseLatLonDistance(g1, g2);
}

MutableMatrix44D FlatMercatorPlanet::createGeodeticTransformMatrix(const Geodetic3D& position) const {
  return MutableMatrix44D::createTranslationMatrix( toCartesian(position) );
}

void FlatMercatorPlanet::beginSingleDrag(const Vector3D& origin, const Vector3D& initialRay) const {
...
MutableMatrix44D FlatMercatorPlanet::singleDrag(const Vector3D& finalRay) const {
...
Effect* FlatMercatorPlanet::createEffectFromLastSingleDrag() const {
  if (!_validSingleDrag) return NULL;
  return new SingleTranslationEffect(_lastDirection.asVector3D());
}

void FlatMercatorPlanet::beginDoubleDrag(const Vector3D& origin,
...
MutableMatrix44D FlatMercatorPlanet::doubleDrag(const Vector3D& finalRay0,
...
Effect* FlatMercatorPlanet::createDoubleTapEffect(const Vector3D& origin,
                                          const Vector3D& centerRay,
...
double FlatMercatorPlanet::distanceToHorizon(const Vector3D& position) const {
...
MutableMatrix44D FlatMercatorPlanet::drag(const Geodetic3D& origin, const Geodetic3D& destination) const {
...
void FlatMercatorPlanet::applyCameraConstrainers(const Camera* previousCamera,
