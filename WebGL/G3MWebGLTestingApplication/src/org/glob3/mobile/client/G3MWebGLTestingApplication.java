...
public class G3MWebGLTestingApplication implements EntryPoint {
	...
	public void onModuleLoad() {
		 ...
		 * 
		 * @Override public void execute() { runUserPlugin(); }
		 * 
		 * });
		 */
	...
	// if (true) {
	// final Sector spain = Sector.fromDegrees(27.3174927, -18.5284423,
	// 45.0299024, 5.4084426);
	// builder.setShownSector(spain);
	// }

	/*
	 * final boolean useMarkers = true; if (useMarkers) { // marks renderer
	 * final boolean readyWhenMarksReady = false; final MarksRenderer
	 * marksRenderer = new MarksRenderer(readyWhenMarksReady);
	 * 
	 ...
	 * true);
	 * 
	 * final Mark m1 = new Mark("Label", new
	 * URL("http://glob3m.glob3mobile.com/icons/markers/g3m.png", false), // new
	 * Geodetic3D(Angle.fromDegrees(28.05), Angle.fromDegrees(-14.36), 0),
	 * AltitudeMode.RELATIVE_TO_GROUND); // m1->addTouchListener(listener);
	 * marksRenderer.addMark(m1);
	 * 
	 * final Mark m2 = new Mark( // "Las Palmas", // new
	 * URL("http://glob3m.glob3mobile.com/icons/markers/g3m.png", false), // new
	 * Geodetic3D(Angle.fromDegrees(28.05), Angle.fromDegrees(-15.36), 0), //
	 * AltitudeMode.RELATIVE_TO_GROUND, 0, // false); //
	 * m2->addTouchListener(listener); marksRenderer.addMark(m2);
	 * 
	 * final Mark m3 = new Mark( // "Washington, DC", // new
	 * Geodetic3D(Angle.fromDegreesMinutesSeconds(38, 53, 42.24),
	 * Angle.fromDegreesMinutesSeconds(-77, 2, 10.92), 100), //
	 * AltitudeMode.RELATIVE_TO_GROUND, 0); marksRenderer.addMark(m3);
	 * 
	 * final boolean randomMarkers = false; if (randomMarkers) { for (int i = 0;
	 * i < 500; i++) { final Angle latitude =
	 * Angle.fromDegrees((Random.nextInt() % 180) - 90); final Angle longitude =
	 * Angle.fromDegrees((Random.nextInt() % 360) - 180); //
	 * NSLog(@"lat=%f, lon=%f", latitude.degrees(), // longitude.degrees());
	 * 
	 * marksRenderer.addMark(new Mark( // "Random", // new
	 * URL("http://glob3m.glob3mobile.com/icons/markers/g3m.png", false), // new
	 * Geodetic3D(latitude, longitude, 0), AltitudeMode.RELATIVE_TO_GROUND)); }
	 * } builder.addRenderer(marksRenderer); }
	 * 
	 * // builder.setInitializationTask(createMarkersInitializationTask());
	 * 
	 * final String proxy = ""; final Downloader_WebGL downloader = new
	 * Downloader_WebGL(8, 10, proxy); builder.setDownloader(downloader);
	 * 
	 * // test bson parser and 3D model final ShapesRenderer shapeRenderer = new
	 * ShapesRenderer(); builder.addRenderer(shapeRenderer);
	 * 
	 * final QuadShape quad = new QuadShape(new
	 * Geodetic3D(Angle.fromDegrees(28.116956), Angle.fromDegrees(-15.440453),
	 * 2000), // AltitudeMode.RELATIVE_TO_GROUND, 10000, 10000, Color.red(),
	 * true);
	 * 
	 * final QuadShape quad2 = new QuadShape( new
	 * Geodetic3D(Angle.fromDegrees(28), Angle.fromDegrees(-15.440453), 2000),
	 * // AltitudeMode.RELATIVE_TO_GROUND, new
	 * URL("http://serdis.dis.ulpgc.es/~a044526/android/glob3.png", false),
	 * 10000, 10000, true);
	 * 
	 * shapeRenderer.addShape(quad); shapeRenderer.addShape(quad2);
	 * 
	 * quad.setPitch(Angle.fromDegrees(90));
	 * quad.setHeading(Angle.fromDegrees(0)); quad.setAnimatedPosition( //
	 * TimeInterval.fromSeconds(60), // new
	 * Geodetic3D(Angle.fromDegrees(28.127222), Angle.fromDegrees(-15.431389),
	 * 10000), // Angle.fromDegrees(90), // Angle.fromDegrees(720),
	 * Angle.zero());
	 * 
	 * quad2.setPitch(Angle.fromDegrees(90));
	 * quad2.setHeading(Angle.fromDegrees(0)); quad2.setAnimatedPosition( //
	 * TimeInterval.fromSeconds(60), // new Geodetic3D(Angle.fromDegrees(28),
	 * Angle.fromDegrees(-15.431389), 10000), // Angle.fromDegrees(90), //
	 ...
	 * 
	 * @Override public ArrayList<GEOSymbol> createSymbols(final
	 * GEO2DPointGeometry geometry) { return new ArrayList<GEOSymbol>(0); }
	 * 
	 * 
	 * private GEO2DLineRasterStyle createLineRasterStyle(final GEOGeometry
	 * geometry) { final JSONObject properties =
	 * geometry.getFeature().getProperties();
	 * 
	 * final String type = properties.getAsString("type", "");
	 * 
	 * final float dashLengths[] = { 1, 12 }; final int dashCount = 2;
	 * 
	 * final Color color = type.equalsIgnoreCase("Water Indicator") // ?
	 * Color.fromRGBA(1, 1, 1, 0.9f) // : Color.fromRGBA(1, 1, 0, 0.9f);
	 * 
	 * return new GEO2DLineRasterStyle( // color, // 8, // StrokeCap.CAP_ROUND,
	 * // StrokeJoin.JOIN_ROUND, // 1, dashLengths, // dashCount, // 0); }
	 * 
	 * 
	 * @Override public ArrayList<GEOSymbol> createSymbols(final
	 * GEO2DLineStringGeometry geometry) { final ArrayList<GEOSymbol> symbols =
	 * new ArrayList<GEOSymbol>(); symbols.add(new
	 * GEORasterLineSymbol(geometry.getCoordinates(),
	 * createLineRasterStyle(geometry))); return symbols; }
	 * 
	 * 
	 * @Override public ArrayList<GEOSymbol> createSymbols(final
	 * GEO2DMultiLineStringGeometry geometry) { final ArrayList<GEOSymbol>
	 * symbols = new ArrayList<GEOSymbol>(); symbols.add(new
	 * GEOMultiLineRasterSymbol(geometry.getCoordinatesArray(),
	 * createLineRasterStyle(geometry))); return symbols; }
	 * 
	 * 
	 * private GEO2DLineRasterStyle createPolygonLineRasterStyle(final
	 * GEOGeometry geometry) { final JSONObject properties =
	 * geometry.getFeature().getProperties();
	 * 
	 * final int colorIndex = (int) properties.getAsNumber("mapcolor7", 0);
	 * 
	 * final Color color = Color.fromRGBA(0.7f, 0, 0, 0.5f).wheelStep(7,
	 * colorIndex).muchLighter().muchLighter();
	 * 
	 * final float dashLengths[] = {}; final int dashCount = 0;
	 * 
	 * return new GEO2DLineRasterStyle(color, 2, StrokeCap.CAP_ROUND,
	 * StrokeJoin.JOIN_ROUND, 1, dashLengths, dashCount, 0); }
	 * 
	 * 
	 * private GEO2DSurfaceRasterStyle createPolygonSurfaceRasterStyle(final
	 * GEOGeometry geometry) { final JSONObject properties =
	 * geometry.getFeature().getProperties();
	 * 
	 * final int colorIndex = (int) properties.getAsNumber("mapcolor7", 0);
	 ...
	 * 
	 * @Override public ArrayList<GEOSymbol> createSymbols(final
	 * GEO2DPolygonGeometry geometry) { final ArrayList<GEOSymbol> symbols = new
	 * ArrayList<GEOSymbol>(0);
	 * 
	 ...
	 * 
	 * @Override public ArrayList<GEOSymbol> createSymbols(final
	 * GEO2DMultiPolygonGeometry geometry) { final ArrayList<GEOSymbol> symbols
	 * = new ArrayList<GEOSymbol>(0);
	 * 
	 * final GEO2DLineRasterStyle lineStyle =
	 * createPolygonLineRasterStyle(geometry); final GEO2DSurfaceRasterStyle
	 * surfaceStyle = createPolygonSurfaceRasterStyle(geometry);
	 * 
	 * final ArrayList<GEO2DPolygonData> polygonsData =
	 * geometry.getPolygonsData(); final int polygonsDataSize =
	 * polygonsData.size();
	 * 
	 * for (int i = 0; i < polygonsDataSize; i++) { final GEO2DPolygonData
	 * polygonData = polygonsData.get(i); symbols.add(new
	 * GEORasterPolygonSymbol(polygonData, lineStyle, surfaceStyle));
	 * 
	 * }
	 * 
	 * return symbols; } };
	 ...
	 * 
	 * final ShapesRenderer shapesRenderer = null; final MarksRenderer
	 * marksRenderer = null; final GEORenderer geoRenderer = new GEORenderer( //
	 * defaultSymbolizer, // meshRenderer, // shapesRenderer, // marksRenderer,
	 * // geoTileRasterizer);
	 * 
	...
	// new URL("http://glob3m.glob3mobile.com/test/aircraft-A320/A320.bson",
	// false), //
	// 0, //
	...
	//
	// if (true) {
	// final URL planeFilePath = new
	// URL("http://serdis.dis.ulpgc.es/~a044526/seymour-plane.json", false);
	// final IBufferDownloadListener listenerPlane = new
	...
	// Angle.fromDegrees(-15.431389), 10000), //
	// AltitudeMode.ABSOLUTE);
	//
	 ...
	 * "proxy.php?url=" builder.setDownloader(new Downloader_WebGL( // 8, //
	 * maxConcurrentOperationCount 10, // delayMillis proxy));
	 * 
	 * _widget = builder.createWidget(); g3mWidgetHolder.add(_widget);
	 */

	/*
	 * private LayerSet createLayerSet() { final LayerSet layerSet = new
	 * LayerSet(); //
	 ...
	 * "http://192.168.1.2/ne_10m_admin_0_countries-Levels10/{level}/{y}/{x}.geojson"
	 * ; final int firstLevel = 2; final int maxLevel = 10;
	 * 
	 ...
	 * // symbolizer, // urlTemplate, // Sector.fullSphere(), // sector
	 * firstLevel, // maxLevel, // TimeInterval.fromDays(30), // timeToCache
	 * true, // readExpired 1, // transparency null // condition );
	 ...
	 * final Panel g3mWidgetHolder = RootPanel.get(_g3mWidgetHolderId);
	 * 
	 * g3mWidgetHolder.setWidth("640px"); g3mWidgetHolder.setHeight("480px");
	 * 
	 ...
	 * 
	 * final int delayMillis = 10; final String proxy = ""; final IDownloader
	 * downloader = new Downloader_WebGL(8, delayMillis, proxy);
	 * 
	 * final IThreadUtils threadUtils = new ThreadUtils_WebGL(delayMillis);
	 * 
	 * final Planet planet = Planet.createEarth();
	 * 
	 * final ArrayList<ICameraConstrainer> cameraConstraints = new
	 * ArrayList<ICameraConstrainer>(); cameraConstraints.add(new
	 * SimpleCameraConstrainer());
	 * 
	 * final CameraRenderer cameraRenderer = new CameraRenderer(); final boolean
	 * useInertia = true; cameraRenderer.addHandler(new
	 * CameraSingleDragHandler(useInertia)); final boolean
	 * allowRotationInDoubleDrag = false; cameraRenderer.addHandler(new
	 * CameraDoubleDragHandler(allowRotationInDoubleDrag));
	 ...
	 * final LayerSet layerSet = new LayerSet();
	 * 
	 * final boolean testingTransparencies = true; if (testingTransparencies) {
	 ...
	 * TimeInterval.fromDays(30), true, new LayerTilesRenderParameters(
	 * Sector.fullSphere(), 2, 4, 0, 6,
	 * LayerTilesRenderParameters.defaultTileTextureResolution(),
	 * LayerTilesRenderParameters.defaultTileMeshResolution(), false));
	 * layerSet.addLayer(blueMarble);
	 * 
	 * final WMSLayer i3Landsat = new WMSLayer("esat", new
	 * URL("http://data.worldwind.arc.nasa.gov/wms?", false),
	 * WMSServerVersion.WMS_1_1_0, Sector.fullSphere(), "image/jpeg",
	 * "EPSG:4326", "", false, new LevelTileCondition(7, 100),
	 * TimeInterval.fromDays(30), true, new LayerTilesRenderParameters(
	 * Sector.fullSphere(), 2, 4, 0, 12,
	 * LayerTilesRenderParameters.defaultTileTextureResolution(),
	 * LayerTilesRenderParameters.defaultTileMeshResolution(), false));
	 ...
	 * "image/png", "EPSG:4326", "", true, null, TimeInterval.fromDays(30),
	 * true, null, (float) 0.5); layerSet.addLayer(pnoa); }
	 * 
	 * final boolean blueMarble = false; if (blueMarble) { final WMSLayer
	 * blueMarbleL = new WMSLayer( // "bmng200405", // new
	 * URL("http://www.nasa.network.com/wms?", false), //
	 * WMSServerVersion.WMS_1_1_0, // Sector.fullSphere(), // "image/jpeg", //
	 * "EPSG:4326", // "", // false, // // new LevelTileCondition(0, 6), null,
	 * // TimeInterval.fromDays(30), // true); layerSet.addLayer(blueMarbleL); }
	 * 
	 * final boolean useOrtoAyto = false; if (useOrtoAyto) {
	 * 
	 * final LayerTilesRenderParameters ltrp = new
	 * LayerTilesRenderParameters(Sector.fullSphere(), 2, 4, 0, 19, new
	 * Vector2I(256, 256),
	 * LayerTilesRenderParameters.defaultTileMeshResolution(), false);
	 * 
	 * final WMSLayer ortoAyto = new WMSLayer( // "orto_refundida", // new
	 * URL("http://195.57.27.86/wms_etiquetas_con_orto.mapdef?", false), //
	 * WMSServerVersion.WMS_1_1_0, // new Sector(new
	 * Geodetic2D(Angle.fromDegrees(39.350228), Angle.fromDegrees(-6.508713)),
	 * new Geodetic2D( Angle.fromDegrees(39.536351),
	 * Angle.fromDegrees(-6.25946))), // "image/jpeg", // "EPSG:4326", // "", //
	 * false, // new LevelTileCondition(4, 19), // TimeInterval.fromDays(30), //
	 * true, // ltrp); layerSet.addLayer(ortoAyto); }
	 * 
	 * final boolean useBing = false; if (useBing) { final WMSLayer bing = new
	 * WMSLayer( // "ve", // new
	 ...
	 * true); layerSet.addLayer(bing); } final boolean useOSMLatLon = false; if
	 * (useOSMLatLon) { // final WMSLayer osm = new WMSLayer( // // "osm", // //
	 * new URL("http://wms.latlon.org/", false), // //
	 * WMSServerVersion.WMS_1_1_0, // // Sector.fromDegrees(-85.05, -180.0,
	 * 85.5, 180.0), // // "image/jpeg", // // "EPSG:4326", // // "", // //
	 * false, // // null); // layerSet.addLayer(osm);
	 * 
	 * final WMSLayer osm = new WMSLayer( // "osm_auto:all", // new
	 * URL("http://129.206.228.72/cached/osm", false), //
	 * WMSServerVersion.WMS_1_1_0, // // Sector.fromDegrees(-85.05, -180.0,
	 * 85.05, 180.0), // Sector.fullSphere(), // "image/jpeg", // "EPSG:4326",
	 * // "", // false, // // new LevelTileCondition(3, 100)); null, //
	 * TimeInterval.fromDays(30), // true); layerSet.addLayer(osm); }
	 * 
	 * final boolean usePnoa = false; if (usePnoa) { final WMSLayer pnoa = new
	 * WMSLayer( // "PNOA", // new URL("http://www.idee.es/wms/PNOA/PNOA",
	 * false), // WMSServerVersion.WMS_1_1_0, Sector.fromDegrees(21, -18, 45,
	 * 6), // "image/png", // "EPSG:4326", // "", // true, // null, //
	 * TimeInterval.fromDays(30), // true); layerSet.addLayer(pnoa); }
	 * 
	 * final boolean testURLescape = false; if (testURLescape) { final WMSLayer
	 * ayto = new WMSLayer(URL.escape("Ejes de via"), // new
	 * URL("http://sig.caceres.es/wms_callejero.mapdef?", false), //
	 * WMSServerVersion.WMS_1_1_0,// Sector.fullSphere(), // "image/png", //
	 * "EPSG:4326", // "", // true, // null, // TimeInterval.fromDays(30), //
	 * true); layerSet.addLayer(ayto); }
	 * 
	 * final PlanetRendererBuilder tlBuilder = new PlanetRendererBuilder();
	 * tlBuilder.setLayerSet(layerSet); tlBuilder.setRenderDebug(true); final
	 * PlanetRenderer planetRenderer = tlBuilder.create();
	 * mainRenderer.addRenderer(planetRenderer);
	 * 
	 * final boolean useQuadShapes = true; if (useQuadShapes) { final
	 * ShapesRenderer shapesRenderer = new ShapesRenderer();
	 * 
	 * // final String textureFileName = "g3m-marker.png"; // final IImage
	 * textureImage = //
	 * IFactory.instance().createImageFromFileName(textureFileName); // // final
	 * QuadShape quad = new QuadShape( // // new
	 * Geodetic3D(Angle.fromDegrees(37.78333333), // //
	 * Angle.fromDegrees(-122.41666666666667), // // 10000), // // textureImage,
	 * // // true, // // textureFileName, // // 500000, // // 500000); //
	 * quad.setHeading(Angle.fromDegrees(0)); //
	 * quad.setPitch(Angle.fromDegrees(0)); // shapesRenderer.addShape(quad);
	 * 
	 * final Geodetic3D circlePosition = new Geodetic3D( //
	 * Angle.fromDegrees(37.78333333), //
	 * Angle.fromDegrees(-122.41666666666667), // 8000);
	 * 
	 * // circle.setHeading(Angle.fromDegrees(45)); //
	 * circle.setPitch(Angle.fromDegrees(45)); // circle.setScale(2.0, 0.5, 1);
	 * // circle.setRadius(circleRadius);
	 * 
	 * final Color circleColor = Color.newFromRGBA(1, 1, 0, 0.5f); final Shape
	 * circle = new CircleShape(circlePosition, AltitudeMode.RELATIVE_TO_GROUND,
	 * 50000, circleColor); shapesRenderer.addShape(circle);
	 * 
	 * final Geodetic3D boxPosition = new
	 * Geodetic3D(Angle.fromDegrees(37.78333333), //
	 * Angle.fromDegrees(-122.41666666666667), // 45000); final Vector3D size =
	 * new Vector3D(20000, 30000, 50000); final Color boxColor =
	 * Color.newFromRGBA(0, 1, 0, 0.5f); final Color edgeColor =
	 * Color.newFromRGBA(0.75f, 0, 0, 0.75f); final Shape box = new
	 * BoxShape(boxPosition, AltitudeMode.RELATIVE_TO_GROUND, size, 2, boxColor,
	 * edgeColor); shapesRenderer.addShape(box);
	 * 
	 ...
	 * 
	 * 
	 * final boolean testingImagesCombine = false; if (testingImagesCombine) {
	 * class DL extends IImageDownloadListener {
	 * 
	 * @Override public void onDownload(final URL url, final IImage image, final
	 * boolean expired) {
	 * 
	 * final int w = image.getWidth(); final int h = image.getHeight();
	 * 
	 * final java.util.ArrayList<IImage> images = new ArrayList<IImage>();
	 * images.add(image); images.add(image);
	 * 
	 * final java.util.ArrayList<RectangleF> srcRs = new
	 * ArrayList<RectangleF>(); srcRs.add(new RectangleF(0, 0, image.getWidth(),
	 * image.getHeight())); srcRs.add(new RectangleF(10, 0, image.getWidth() -
	 * 10, image.getHeight()));
	 * 
	 * final java.util.ArrayList<RectangleF> destRs = new
	 * ArrayList<RectangleF>(); destRs.add(new RectangleF(0, 0, 256, 256));
	 * destRs.add(new RectangleF(50, 20, 256, 70));
	 * 
	 * final java.util.ArrayList<Float> transparencies = new ArrayList<Float>();
	 * transparencies.add((float) 1.0); transparencies.add((float) 0.5);
	 * 
	 * class QuadListener extends IImageListener { ShapesRenderer _sr;
	 * 
	 * 
	 * public QuadListener(final ShapesRenderer sr) { _sr = sr; }
	 * 
	 * 
	 * @Override public void imageCreated(final IImage image2) { final Shape
	 * quadImages = new QuadShape(new Geodetic3D(Angle.fromDegrees(28.410728),
	 * Angle.fromDegrees(-16.339417), 8000), AltitudeMode.RELATIVE_TO_GROUND,
	 * image2, 50000, 50000, true);
	 * 
	 * _sr.addShape(quadImages); } }
	 * 
	 * IImageUtils.combine(new Vector2I(256, 256), images, srcRs, destRs,
	 * transparencies, new QuadListener( shapesRenderer), true);
	 * 
	 * }
	 * 
	 * 
	 ...
	 * 
	 * 
	 * @Override public void onCanceledDownload(final URL url, final IImage
	 * image, final boolean expired) { }
	 * 
	 * }
	 * 
	 * downloader.requestImage( // new URL(
	 ...
	 * , false), // 100000, // TimeInterval.fromDays(1), // true, // new DL(),
	 * // true);
	 * 
	 * }
	 * 
	 * }
	 * 
	 * final BusyMeshRenderer busyRenderer = new
	 * BusyMeshRenderer(Color.newFromRGBA(0, 0, 0, 1));
	 * 
	 ...
	 * 
	 * final ArrayList<PeriodicalTask> periodicalTasks = new
	 * ArrayList<PeriodicalTask>();
	 * 
	 ...
	 * 
	 * final InitialCameraPositionProvider initialCameraPositionProvider = new
	 * SimpleInitialCameraPositionProvider();
	 * 
	 * 
	 * // public static final G3MWidget create(final GL gl, // final IStorage
	 * storage, // final IDownloader downloader, // final IThreadUtils
	 * threadUtils, // final ICameraActivityListener cameraActivityListener, //
	 * final Planet planet, // final java.util.ArrayList<ICameraConstrainer>
	 * cameraConstrainers, // final CameraRenderer cameraRenderer, // final
	 * Renderer mainRenderer, // final Renderer busyRenderer, // final
	 * ErrorRenderer errorRenderer, // final Color backgroundColor, // final
	 * boolean logFPS, // final boolean logDownloaderStatistics, // final
	 * GInitializationTask initializationTask, // final boolean
	 * autoDeleteInitializationTask, // final
	 * java.util.ArrayList<PeriodicalTask> periodicalTasks, // final
	 * GPUProgramManager gpuProgramManager, // final SceneLighting
	 * sceneLighting, // final InitialCameraPositionProvider
	 * initialCameraPositionProvider);
	 * 
	 * 
	 * final ErrorRenderer errorRenderer = new HUDErrorRenderer(); final
	 * Renderer hudRenderer = null; _widget.initWidget(// storage, //
	 * downloader, // threadUtils, // cameraActivityListener, // planet, //
	 * cameraConstraints, // cameraRenderer, // mainRenderer, // busyRenderer,
	 * // errorRenderer, // hudRenderer, // backgroundColor, // logFPS, //
	 * logDownloaderStatistics, // initializationTask, //
	 * autoDeleteInitializationTask, // periodicalTasks, // userData, //
	 * lighting, // initialCameraPositionProvider, null);
	 * 
	 * private static GEO2DLineRasterStyle createPolygonLineRasterStyle(final
	 * GEOGeometry geometry) { final JSONObject properties =
	 * geometry.getFeature().getProperties(); final int colorIndex = (int)
	 * properties.getAsNumber("mapcolor7", 0); final Color color =
	 * FROM_COLOR.wheelStep(7, colorIndex).muchLighter().muchLighter(); final
	 * float dashLengths[] = {}; final int dashCount = 0; return new
	 * GEO2DLineRasterStyle(color, 2, StrokeCap.CAP_ROUND,
	 * StrokeJoin.JOIN_ROUND, 1, dashLengths, dashCount, 0);
	 * 
	 * }
	 * 
	 ...
	 * createPolygonSurfaceRasterStyle(final GEOGeometry geometry) { final
	 * JSONObject properties = geometry.getFeature().getProperties(); final int
	 * colorIndex = (int) properties.getAsNumber("mapcolor7", 0); final Color
	 * color = FROM_COLOR.wheelStep(7, colorIndex); return new
	 * GEO2DSurfaceRasterStyle(color); }
	 * 
	 * 
	 * 
	 * //final LayerSet layerSet = new LayerSet();
	 ...
	 * true, // ltrp); layerSet.addLayer(ortoAyto);
	 * 
	 * @Override public GEORasterSymbolizer copy() { return new
	 * SampleRasterSymbolizer(); }
	 * 
	 * 
	 * @Override public ArrayList<GEORasterSymbol> createSymbols(final
	 * GEO2DPointGeometry geometry) { return null; }
	 * 
	 * 
	 * @Override public ArrayList<GEORasterSymbol> createSymbols(final
	 * GEO2DLineStringGeometry geometry) { return null; }
	 * 
	 * 
	 * 
	 * final boolean testingTransparencies = false; if (testingTransparencies) {
	 * final WMSLayer blueMarble = new WMSLayer("bmng200405", new
	 * URL("http://www.nasa.network.com/wms?", false),
	 * WMSServerVersion.WMS_1_1_0, Sector.fullSphere(), "image/jpeg",
	 * "EPSG:4326", "", false, new LevelTileCondition( 0, 6),
	 * TimeInterval.fromDays(30), true, new
	 * LayerTilesRenderParameters(Sector.fullSphere(), 2, 4, 0, 6,
	 * LayerTilesRenderParameters.defaultTileTextureResolution(),
	 * LayerTilesRenderParameters.defaultTileMeshResolution(), false));
	 * layerSet.addLayer(blueMarble);
	 * 
	 ...
	 * "EPSG:4326", "", false, new LevelTileCondition( 7, 100),
	 * TimeInterval.fromDays(30), true, new
	 * LayerTilesRenderParameters(Sector.fullSphere(), 2, 4, 0, 12,
	 * LayerTilesRenderParameters.defaultTileTextureResolution(),
	 * LayerTilesRenderParameters.defaultTileMeshResolution(), false));
	 * layerSet.addLayer(i3Landsat);
	 * 
	 ...
	 * WMSServerVersion.WMS_1_1_0, Sector.fullSphere(), "image/png",
	 * "EPSG:4326", "countryboundaries", true, null, TimeInterval.fromDays(30),
	 * true); layerSet.addLayer(political);
	 */

	/*
	 * @Override public ArrayList<GEORasterSymbol> createSymbols(final
	 * GEO2DMultiLineStringGeometry geometry) { return null; }
	 * 
	 * 
	 * 
	 * @Override public ArrayList<GEORasterSymbol> createSymbols(final
	 * GEO2DPolygonGeometry geometry) { final ArrayList<GEORasterSymbol> symbols
	 * = new ArrayList<GEORasterSymbol>(); final GEOPolygonRasterSymbol symbol =
	 * new GEOPolygonRasterSymbol( // geometry.getPolygonData(), //
	 * createPolygonLineRasterStyle(geometry), //
	 * createPolygonSurfaceRasterStyle(geometry) // ); symbols.add(symbol);
	 * return symbols; }
	 * 
	 * 
	 * @Override public ArrayList<GEORasterSymbol> createSymbols(final
	 * GEO2DMultiPolygonGeometry geometry) { final ArrayList<GEORasterSymbol>
	 * symbols = new ArrayList<GEORasterSymbol>();
	 * 
	 * final GEO2DLineRasterStyle lineStyle =
	 * createPolygonLineRasterStyle(geometry); final GEO2DSurfaceRasterStyle
	 * surfaceStyle = createPolygonSurfaceRasterStyle(geometry);
...
//	}
	...
	public void testSelectableShape() {
		final G3MWidget_WebGL widgetJS = new G3MWidget_WebGL();
		...
		shapesRenderer
				.loadJSONSceneJS(
						new URL(
								"http://serdis.dis.ulpgc.es/~atrujill/glob3m/IGO/SelectingShapes/seymour-plane.json",
								false),
...
//		shapesRenderer.setShapeTouchListener(myShapeTouchListener, true);
	...
	public void testBILGC() {
		...
		// wms layer
		final LayerTilesRenderParameters ltrp = new LayerTilesRenderParameters(
				Sector.fullSphere(), 2, 4, 0, 19, new Vector2I(256, 256),
		...
		WMSLayer grafcanLIDAR = new WMSLayer("LIDAR_MTL", new URL(
				...
				"image/jpeg", "EPSG:4326", "", false, new LevelTileCondition(0,
						17), TimeInterval.fromDays(30), true, ltrp);
		layerSet.addLayer(grafcanLIDAR);
		builder.getPlanetRendererBuilder().setLayerSet(layerSet);

		// create elevations for GC
		Sector sector = Sector.fromDegrees(27.7116484957735,
				-15.90589160041418, 28.225913322423995, -15.32910937385168);
		Vector2I extent = new Vector2I(1000, 1000); // bil resolution
		URL url = new URL(
				"http://serdis.dis.ulpgc.es/~atrujill/glob3m/Fiware/gc2.bil",
				false);
		...
		Geodetic3D position = new Geodetic3D(Angle.fromDegrees(28.070),
				Angle.fromDegrees(-15.478), 9291.1);
...
//
//		/*
//		 * WMSLayer grafcanLIDAR = new WMSLayer("LIDAR_MTL", new
//		 * URL("http://idecan1.grafcan.es/ServicioWMS/MTL?", false),
//		 * WMSServerVersion.WMS_1_1_0, Sector.fullSphere(),//gcSector,
//		 * "image/jpeg", "EPSG:4326", "", false, new LevelTileCondition(0, 17),
//		 * TimeInterval.fromDays(30), true, ltrp);
//		 * layerSet.addLayer(grafcanLIDAR);
//		 */
//
//		WMSLayer grafcanOrto = new WMSLayer("WMS_OrtoExpress", new URL(
//				"http://idecan1.grafcan.es/ServicioWMS/OrtoExpress?", false),
//				WMSServerVersion.WMS_1_1_0, Sector.fullSphere(), "image/jpeg",
//				"EPSG:4326", "", false, new LevelTileCondition(0, 19),
//				TimeInterval.fromDays(30), true, ltrp);
//		layerSet.addLayer(grafcanOrto);
//		builder.getPlanetRendererBuilder().setLayerSet(layerSet);
//
//		// create elevations for GC
//		Sector sectorGC = Sector.fromDegrees(27.7116484957735,
//				-15.90589160041418, 28.225913322423995, -15.32910937385168);
//		Vector2I extentGC = new Vector2I(1000, 1000); // bil resolution
//		URL urlGC = new URL(
//				"http://serdis.dis.ulpgc.es/~atrujill/glob3m/SenderosGC/resources/gc.bil",
//				false);
//		ElevationDataProvider elevationDataProviderGC = new SingleBilElevationDataProvider(
//				urlGC, sectorGC, extentGC);
//
//		// create elevation for Bandama (5m/pixel)
//		Sector bandamaBilSector = Sector.fromDegrees(28.0186134922002,
//				-15.466485021954, 28.0501903939333, -15.4475303331328);
//		Vector2I extentBandama = new Vector2I(371, 702);
//		URL urlBandama = new URL(
//				"http://serdis.dis.ulpgc.es/~atrujill/glob3m/SenderosGC/resources/mdt1_bandama.bil",
//				false);
//		ElevationDataProvider elevationDataProviderBandama = new SingleBilElevationDataProvider(
//				urlBandama, bandamaBilSector, extentBandama);
//
//		/*
//		 * // create elevation for Bandama (1m/pixel) Sector bandamaBilSector =
//		 * Sector
//		 * .fromDegrees(28.0186134922002,-15.466485021954,28.0501903939333,
//		 * -15.4475303331328); Vector2I extentBandama = new Vector2I(1881,
//		 * 3522); // this url format is for executing in local //URL urlBandama
//		 * = new URL(
//		 * "http://serdis.dis.ulpgc.es/~atrujill/glob3m/SenderosGC/resources/mdt.bil"
//		 * ,false); // this url format is for executing in web URL urlBandama =
//		 * new URL("mdt.bil", false); ElevationDataProvider
//		 * elevationDataProviderBandama = new
//		 * SingleBilElevationDataProvider(urlBandama, bandamaBilSector,
//		 * extentBandama);
//		 */
//
...
//		elevationDataProvider
//				.addElevationDataProvider(elevationDataProviderBandama);
...
//		Geodetic3D position = new Geodetic3D(Angle.fromDegrees(28.0196),
//				Angle.fromDegrees(-15.4589), 603.9);
//		_widget.setCameraPosition(position);
...
//			final int maxLevel = 10;
//			final GEORasterSymbolizer symbolizer = new SampleRasterSymbolizer();
//			final TiledVectorLayer tiledVectorLayer = TiledVectorLayer
//					.newMercator( //
//							symbolizer, //
//							urlTemplate, //
//							Sector.fullSphere(), // sector
...
//									// "proxy.php?url="
//		/*
//		 * builder.setDownloader(new Downloader_WebGL( // 8, //
//		 * maxConcurrentOperationCount 10, // delayMillis proxy));
//		 */
//
//		// For A1800DC
...
//		final GEORenderer renderer1800 = new GEORenderer(symbolizer, mr1, null,
//				null, null);
...
//		final GEORenderer renderer1900 = new GEORenderer(symbolizer, mr2, null,
//				null, null);
...
//				context.getDownloader()
//						.requestBuffer(
//								new URL(
//										"http://serdis.dis.ulpgc.es/~atrujill/glob3m/Geacron/A1800DC.json"),
//								100000000, TimeInterval.forever(), true,
//								listener, false);
//				context.getDownloader()
//						.requestBuffer(
//								new URL(
//										"http://serdis.dis.ulpgc.es/~atrujill/glob3m/Geacron/A1900DC.json"),
//								100000000, TimeInterval.forever(), true,
//								listener, false);
//			}
//
...
//				.fromMilliseconds(1000), new GeaCronTask(mr1, mr2)));
//
//		_widget = builder.createWidget();
//	}
