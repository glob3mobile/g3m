...
public class ByteBuffer_JavaDesktop extends IByteBuffer {
   ...
   @Override
   public ByteBuffer_JavaDesktop copy(final int from, final int length) {
      final byte[] newBuffer = new byte[length];
      System.arraycopy(_buffer, from, newBuffer, 0, length);
      return new ByteBuffer_JavaDesktop(newBuffer);
   }
