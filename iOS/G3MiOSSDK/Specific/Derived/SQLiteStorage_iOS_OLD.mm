...
#include "SQLiteStorage_iOS_OLD.hpp"
#include <sqlite3.h>


SQLiteStorage_iOS_OLD::SQLiteStorage_iOS_OLD(const std::string databaseName,
                                             const std::string table) {
  _databaseName = databaseName;
  _table = table;
  SQLiteStorage_iOS_OLD::createEditableCopyOfDatabaseIfNeeded();
  SQLiteStorage_iOS_OLD::testConnection();
  SQLiteStorage_iOS_OLD::checkDataBaseConnection();
  SQLiteStorage_iOS_OLD::checkTableExist();
}

bool SQLiteStorage_iOS_OLD::contains(const std::string& filename) {
  bool contain = false;
  sqlite3 *db;
...
void SQLiteStorage_iOS_OLD::save(const std::string& filename,
  ...
  if (SQLITE_OK != sqlite3_open(_writableDBPath, &db)) {
    ...
    if (sqlite3_prepare_v2(db, consulta, -1, &ppStmt, NULL) == SQLITE_OK) {
      ...
      sqlite3_bind_blob(ppStmt, sqlite3_bind_parameter_index(ppStmt, "@file"), buffer.getData(), buffer.getLength(), SQLITE_TRANSIENT);
      //sqlite3_step(ppStmt);
...
const ByteBuffer* SQLiteStorage_iOS_OLD::read(const std::string& filename) {
  return SQLiteStorage_iOS_OLD::findFileFromFileName(filename);
}

const ByteBuffer* SQLiteStorage_iOS_OLD::findFileFromFileName(const std::string& filename) {
...
void SQLiteStorage_iOS_OLD::createEditableCopyOfDatabaseIfNeeded() {
...
bool SQLiteStorage_iOS_OLD::checkDataBaseConnection() const {
  sqlite3 *db;
  bool ok = true;
  if(SQLITE_OK != sqlite3_open(_writableDBPath, &db)){
...
bool SQLiteStorage_iOS_OLD::checkTableExist() const {
  sqlite3 *db;
  bool ok = true;
  if(SQLITE_OK != sqlite3_open(_writableDBPath, &db)){
    printf("ERROR Opening Database %s \n", _databaseName.c_str());
...
void SQLiteStorage_iOS_OLD::testConnection() const {
  sqlite3 *db;
  if(SQLITE_OK != sqlite3_open(_writableDBPath, &db)){
