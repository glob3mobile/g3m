...
Vector3D Sphere::geodeticSurfaceNormal(const Angle& latitude,
...
std::vector<double> Sphere::intersectionsDistances(const Vector3D& origin,
...
Geodetic3D Sphere::toGeodetic3D(const Vector3D& position) const {
...
Geodetic2D Sphere::getMidPoint (const Geodetic2D& P0, const Geodetic2D& P1) const
{
...
std::list<Vector3D> Sphere::computeCurve(const Vector3D& start,
                                            const Vector3D& stop,
...
double Sphere::computePreciseLatLonDistance(const Geodetic2D& g1,
...
// compute distance from two points
double Sphere::computeFastLatLonDistance(const Geodetic2D& g1,
                                            const Geodetic2D& g2) const {
  const IMathUtils* mu = IMathUtils::instance();
...
Vector3D Sphere::closestIntersection(const Vector3D& pos,
...
Vector3D Sphere::closestPointToSphere(const Vector3D& pos, const Vector3D& ray) const {
  const IMathUtils* mu = IMathUtils::instance();
...
MutableMatrix44D Sphere::createGeodeticTransformMatrix(const Geodetic3D& position) const {
