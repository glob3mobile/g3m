...
Matrix Matrix::createProjectionMatrix(double left, double right,
  ...
  return Matrix(2 * znear / rl, 0, 0, 0,
                          0, 2 * znear / tb, 0, 0,
...
Matrix Matrix::transposed() const {
  return Matrix(_m00, _m01, _m02, _m03,
                          _m10, _m11, _m12, _m13,
                          _m20, _m21, _m22, _m23,
                          _m30, _m31, _m32, _m33);
}
...
void Matrix::print(const std::string& name, const ILogger* log) const
{
 ...
 */
Vector3D Matrix::unproject(const Vector3D& pixel3D, const int viewport[4]) const {
...
Vector2D Matrix::project(const Vector3D& point, const int viewport[4]) const
{
  double in[4], out[4];
...
Matrix Matrix::createTranslationMatrix(const Vector3D& t){
...
Matrix Matrix::createRotationMatrix(const Angle& angle, const Vector3D& p)
{
  ...
  return Matrix(p0.x() * p0.x() * (1 - c) + c, p0.x() * p0.y() * (1 - c) + p0.z() * s, p0.x() * p0.z() * (1 - c) - p0.y() * s, 0,
...
Matrix Matrix::createGeneralRotationMatrix(const Angle& angle, 
                                          const Vector3D& axis, const Vector3D& point){
  Matrix T1 = Matrix::createTranslationMatrix(point.times(-1.0));
  Matrix R  = Matrix::createRotationMatrix(angle, axis);
  Matrix T2 = Matrix::createTranslationMatrix(point);
  return T2.multiply(R).multiply(T1);
}

Matrix Matrix::createModelMatrix(const MutableVector3D& pos, 
