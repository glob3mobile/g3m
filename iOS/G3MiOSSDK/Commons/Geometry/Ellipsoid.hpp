...
public:
  ...
  Ellipsoid(const Vector3D& center, const Vector3D& radii):
  ...
  Vector3D getRadii() const { return _radii;}
  Vector3D getRadiiSquared() const { return _radiiSquared;}
  Vector3D getRadiiToTheFourth() const { return _radiiToTheFourth;}
  Vector3D getOneOverRadiiSquared() const { return _oneOverRadiiSquared;}
  
  double getMeanRadius() const { return (_radii._x + _radii._y + _radii._y) /3;}
  
  
};

#endif /* defined(__G3MiOSSDK__Ellipsoid__) */
