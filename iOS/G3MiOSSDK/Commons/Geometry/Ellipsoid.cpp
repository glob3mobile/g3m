...
std::vector<double> Ellipsoid::intersectionsDistances(const Vector3D& origin,
  ...
  const double t = -0.5 * (b + (b > 0.0 ? 1.0 : -1.0) * MATH.sqrt(discriminant));
...
Vector3D Ellipsoid::toVector3D(const Geodetic3D& geodetic) const {
  ...
  const double gamma = MATH.sqrt((k.x() * n.x()) +
                            (k.y() * n.y()) +
                            (k.z() * n.z()));
...
Geodetic2D Ellipsoid::toGeodetic2D(const Vector3D& positionOnEllipsoid) const {
  ...
  return Geodetic2D(Angle::fromRadians(MATH.asin(n.z())),
                    Angle::fromRadians(MATH.atan2(n.y(), n.x())));;
}
...
Vector3D Ellipsoid::scaleToGeocentricSurface(const Vector3D& position) const {
  double beta = 1.0 / MATH.sqrt((position.x() * position.x()) * _oneOverRadiiSquared.x() +
                           (position.y() * position.y()) * _oneOverRadiiSquared.y() +
                           (position.z() * position.z()) * _oneOverRadiiSquared.z());
...
double Ellipsoid::computePreciseLatLonDistance(const Geodetic2D& g1,
  ...
  double latQ = medLat / 180 * MATH.pi(), lonQ = medLon / 180 * MATH.pi();
  double coslatP = MATH.cos(latP), sinlatP = MATH.sin(latP);
  double coslonP = MATH.cos(lonP), sinlonP = MATH.sin(lonP);
  double coslatQ = MATH.cos(latQ), sinlatQ = MATH.sin(latQ);
  double coslonQ = MATH.cos(lonQ), sinlonQ = MATH.sin(lonQ);
