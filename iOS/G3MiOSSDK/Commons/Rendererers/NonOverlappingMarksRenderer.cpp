...
double NonOverlappingMark::updatePositionWithCurrentForce(float timeInSeconds,
                                                          int viewportWidth,
                                                          int viewportHeight,
  ...
  double d = widgetPosition.sub(_widget->getScreenPos()).length();
...
double NonOverlappingMarksRenderer::applyForces(long long now, const Camera* camera) {
  
  double maxDis = -1; //Meaning no forces have been applied
  ...
  if (_lastPositionsUpdatedTime != 0) { //If not First frame
    ...
    const size_t visibleMarksSize = _visibleMarks.size();
    for (size_t i = 0; i < visibleMarksSize; i++) {
      double distance = _visibleMarks[i]->updatePositionWithCurrentForce(timeInSeconds,
                                                                         viewPortWidth,
                                                                         viewPortHeight,
                                                                         _viewportMargin);
