...
void MarkWidget::render(const G3MRenderContext *rc, GLState *glState) {
  rc->getGL()->drawArrays(GLPrimitive::triangleStrip(), 0, 4, _glState, *rc->getGPUProgramManager(), REGULAR_RENDER);
}
...
void NonOverlappingMarksRenderer::renderConnectorLines(const G3MRenderContext* rc) {
  ...
  rc->getGL()->drawArrays(GLPrimitive::lines(), 0, pos2D.size()/2, _connectorsGLState, *rc->getGPUProgramManager(), REGULAR_RENDER);
}
