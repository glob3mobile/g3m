...
OLDHUDRenderer::OLDHUDRenderer() :
_glState(new GLState())
{
}

OLDHUDRenderer::ShownImage::~ShownImage() {
  _factory->deleteImage(_image);
  //  delete _image;
  delete _mesh;
}

void OLDHUDRenderer::ShownImage::clearMesh() {
  delete _mesh;
  _mesh = NULL;
}

void OLDHUDRenderer::onResizeViewportEvent(const G3MEventContext* ec,
                                           int width,
                                           int height) {
...
Mesh* OLDHUDRenderer::ShownImage::createMesh(const G3MRenderContext* rc) const {
  //TEXTURED
#ifdef C_CODE
...
void OLDHUDRenderer::render(const G3MRenderContext* rc,
                            GLState* glState) {
  const int size = _images.size();
  for (int i = 0; i < size; i++) {
