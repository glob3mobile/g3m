...
void PlanetRenderer::render(const G3MRenderContext* rc,
  ...
  if (_firstRender && _tilesRenderParameters->_forceFirstLevelTilesRenderOnStart) {
    ...
    while (!_toVisit.empty()) {
      ...
      size_t toVisitSize = _toVisit.size();
...
void PlanetRenderer::zRender(const G3MRenderContext* rc, GLState* glState){
  ...
  if (_tilesRendering.size() < 2){
    const Tile* tile = _tilesRendering[0];
    if (!tile->isElevationDataSolved() || !tile->isTextureSolved()){
      return;
    }
  }
  
  const size_t nt = _tilesRendering.size();
  for (int i = 0; i < nt; i++) {
    _tilesRendering[i]->zRender(rc, *zRenderGLState);
  }
  
  for (std::vector<Tile*>::iterator iter = _tilesRendering.begin();
       iter != _tilesRendering.end();
       iter++) {
    Tile* tile = *iter;
