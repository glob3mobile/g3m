...
GEO3DCoordinatesArrayData::GEO3DCoordinatesArrayData(std::vector<std::vector<Geodetic3D*>*>* coordinatesArray) :
_sector(NULL)
{
  if (coordinatesArray == NULL) {
    ...
    _coordinatesArray = new std::vector<const GEO3DCoordinatesData*>;
    const size_t size = coordinatesArray->size();
    for (size_t i = 0; i < size; i++) {
      std::vector<Geodetic3D*>* coordinates = coordinatesArray->at(i);
      if (coordinates != NULL) {
        _coordinatesArray->push_back( new GEO3DCoordinatesData(coordinates) );
      }
    }
  }
}

GEO3DCoordinatesArrayData::~GEO3DCoordinatesArrayData() {
  if (_coordinatesArray != NULL) {
    ...
    for (size_t i = 0; i < coordinatesArrayCount; i++) {
      const GEO3DCoordinatesData* coordinates = _coordinatesArray->at(i);
      //if (coordinates != NULL) {
...
Sector* GEO3DCoordinatesArrayData::calculateSector() const {
  ...
  for (size_t i = 0; i < coordinatesArrayCount; i++) {
    ...
    for (size_t j = 0; j < coordinatesCount; j++) {
      const Geodetic3D* coordinate = coordinates->get(j);
...
const Sector* GEO3DCoordinatesArrayData::getSector() const {
  if (_sector == NULL) {
...
long long GEO3DCoordinatesArrayData::getCoordinatesCount() const {
  long long result = 0;
  if (_coordinatesArray != NULL) {
    const size_t coordinatesArrayCount = _coordinatesArray->size();
    for (size_t i = 0; i < coordinatesArrayCount; i++) {
