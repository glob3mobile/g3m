...
void G3MWidget::addNextPassRenderer(Renderer* r){
...
RenderState G3MWidget::calculateRendererState() {
    ...
    for (size_t i = 0; i < _multiPassRenderers.size(); ++i){
        Renderer* mpr = _multiPassRenderers[i];
...
void G3MWidget::rawRenderStereoParallelAxis(const RenderState_Type renderStateType) {
    ...
    if (eyesUpdated) {
        ...
        if (renderStateType == RENDER_READY) {
            ...
            const double eyesSeparation = _interOcularDistance;
...
void G3MWidget::onPause() {
    ...
    for (size_t i = 0; i < _multiPassRenderers.size(); ++i){
        _multiPassRenderers[i]->onPause(_context);
    }
...
void G3MWidget::onResume() {
    ...
    for (size_t i = 0; i < _multiPassRenderers.size(); ++i){
        _multiPassRenderers[i]->onResume(_context);
    }
...
void G3MWidget::onDestroy() {
    ...
    for (size_t i = 0; i < _multiPassRenderers.size(); ++i){
        _multiPassRenderers[i]->onDestroy(_context);
    }
