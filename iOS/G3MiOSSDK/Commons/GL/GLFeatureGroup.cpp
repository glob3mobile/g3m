...
void GLFeatureCameraGroup::apply(const GLFeatureSet& features,
                                 GPUVariableValueSet& vs,
                                 GLGlobalState& state) {
    Matrix44DMultiplicationHolderBuilder modelViewHolderBuilder; //Model + View + Projection
    Matrix44DMultiplicationHolderBuilder modelTransformHolderBuilder; //Model
    Matrix44DMultiplicationHolderBuilder modelHolderBuilder; //View
    ...
    for (int i = 0; i < featuresSize; i++) {
        ...
        if (group == CAMERA_GROUP) {
            ...
            if (id == GLF_TRANSPARENCY_DISTANCE_THRESHOLD){
                modelRequired = true;
            }
        }
    }
