...
public class BerkeleyDBOctreeNode implements PersistentOctree.Node {
	...
	static Angle calculateSplitLatitude(final Angle lowerLatitude,
		...
		final double middleV = (getMercatorV(lowerLatitude) / 2.0)
				+ (getMercatorV(upperLatitude) / 2.0);
	...
	private BerkeleyDBOctreeNode(final BerkeleyDBOctree octree,
			final byte[] id, final Sector sector, final int pointsCount,
			final Geodetic3D averagePoint, final Format format) {
		_octree = octree;
		_id = id;
		_sector = sector;
	...
	private BerkeleyDBOctreeNode(final BerkeleyDBOctree octree,
			final byte[] id, final Sector sector, final PointsSet pointsSet) {
		_octree = octree;
		_id = id;
		_sector = sector;
	...
	private static BerkeleyDBOctreeNode getAncestorOrSameLevel(
			final Transaction txn, final BerkeleyDBOctree octree,
			final byte[] id) {
		byte[] ancestorId = id;
		while (ancestorId != null) {
			final BerkeleyDBOctreeNode ancestor = octree.readNode(txn,
					ancestorId, true);
			if (ancestor != null) {
				return ancestor;
			}
	...
	private static List<BerkeleyDBOctreeNode> getDescendants(
			final Transaction txn, final BerkeleyDBOctree octree,
			final byte[] id, final boolean loadPoints) {
		...
		try (final Cursor cursor = nodeDB.openCursor(txn, cursorConfig)) {
			...
			if (status == OperationStatus.SUCCESS) {
				...
				if (Utils.isGreaterThan(key, id)) {
					result.add(fromDB(txn, octree, key, dataEntry.getData(),
							loadPoints));
				}
				...
				while (cursor.getNext(keyEntry, dataEntry, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
					...
					if (Utils.isGreaterThan(key, id)) {
						result.add(fromDB(txn, octree, keyEntry.getData(),
	...
	static void insertPoints(final Transaction txn,
			final BerkeleyDBOctree octree, final TileHeader header,
			final PointsSet pointsSet) {
		final byte[] id = header._id;
		// Only store on leaf nodes so make sure there is no ancestor
		final BerkeleyDBOctreeNode ancestor = getAncestorOrSameLevel(txn,
				octree, id); // Same level - place you are trying to add is
								// already on disk
		if (ancestor != null) {
			// System.out.println("==> found ancestor (" + ancestor.getID() +
			// ") for tile " + toString(id));
			// if there is an ancestor or there is already on disk...
			ancestor.mergePoints(txn, pointsSet);
			return;
		}

		final List<BerkeleyDBOctreeNode> descendants = getDescendants(txn,
				octree, id, false);
		if ((descendants != null) && !descendants.isEmpty()) {
			splitPointsIntoDescendants(txn, octree, header, pointsSet,
					descendants);
			return;
		}
		// if no ancestor and no descendants, save into candidate
		final BerkeleyDBOctreeNode tile = new BerkeleyDBOctreeNode(octree, id,
				header._sector, pointsSet);
		tile.rawSave(txn);
	}
	...
	private static void splitPointsIntoDescendants(final Transaction txn,
		...
		final List<Geodetic3D> points = new ArrayList<Geodetic3D>(
				pointsSet._points);
		for (final BerkeleyDBOctreeNode descendant : descendants) {
			final PointsSet descendantPointsSet = extractPoints(
					descendant._sector, points);
			if (descendantPointsSet != null) {
				// System.out.println(">>> tile " + toString(header._id) +
				// " split " + descendantPointsSet.size()
				// + " points into descendant " + toString(descendant._id) +
				// " (" + points.size()
		...
		if (!points.isEmpty()) {
			final List<TileHeader> descendantsHeaders = descendantsHeadersOfLevel(
					header, header.getLevel() + 1);
			for (final TileHeader descendantHeader : descendantsHeaders) {
				final PointsSet descendantPointsSet = extractPoints(
						descendantHeader._sector, points);
				if (descendantPointsSet != null) {
					// System.out.println(">>> 2ND tile " + toString(header._id)
					// + " split " + descendantPointsSet.size()
					// + " points into descendant " +
					// toString(descendantHeader._id) + " (" + points.size()
					// + " points not yet distributed)");

					insertPoints(txn, octree, descendantHeader,
							descendantPointsSet);
				}
			}
	...
	private static List<TileHeader> descendantsHeadersOfLevel(
			final TileHeader header, final int level) {
		final List<TileHeader> result = new ArrayList<TileHeader>();
		descendantsHeadersOfLevel(result, level, header);
		return result;
	}

	private static void descendantsHeadersOfLevel(
			final List<TileHeader> result, final int level,
	...
	private void rawSave(final Transaction txn) {
		...
		if (getPoints().size() >= _octree.getMaxPointsPerTile()) {
			System.out.println("***** logic error, tile " + getID()
					+ " has more points than threshold (" + getPoints().size()
		...
		OperationStatus status = nodeDB.put(txn, key, new DatabaseEntry(
				createNodeEntry(format)));
		if (status != OperationStatus.SUCCESS) {
			throw new RuntimeException("Status not supported: " + status);
		}
		status = nodeDataDB.put(txn, key, new DatabaseEntry(
				createNodeDataEntry(format)));
		if (status != OperationStatus.SUCCESS) {
			throw new RuntimeException("Status not supported: " + status);
		}
	...
	private static BerkeleyDBOctreeNode getAncestor(final Transaction txn,
			final BerkeleyDBOctree octree, final byte[] id) {
	...
	private static PointsSet extractPoints(final Sector sector,
		...
		final double averageLatitudeInRadians = sumLatitudeInRadians
				/ extractedSize;
		final double averageLongitudeInRadians = sumLongitudeInRadians
				/ extractedSize;
	...
	private void split(final Transaction txn, final PointsSet newPointsSet) {
		...
		final List<Geodetic3D> mergedPoints = new ArrayList<Geodetic3D>(
				mergedPointsSize);
		...
		final TileHeader[] children = header.createChildren();
		for (final TileHeader child : children) {
			final PointsSet childPointsSet = extractPoints(child._sector,
					mergedPoints);
			if (childPointsSet != null) {
				// System.out.println(">>> tile " + getID() + " split " +
				// childPointsSet.size() + " points into " +
				// toString(child._id));
	...
	private static double weightedAverage(final double value1,
			final int count1, final double value2, final int count2) {
		return ((value1 * count1) + (value2 * count2)) / (count1 + count2);
	}

	private static Angle weightedAverage(final Angle value1, final int count1,
			final Angle value2, final int count2) {
		return Angle.fromRadians(weightedAverage(value1._radians, count1,
				value2._radians, count2));
	}

	private static Geodetic3D weightedAverage(final Geodetic3D value1,
			final int count1, final Geodetic3D value2, final int count2) {
	...
	private void updateFromPoints(final Transaction txn,
		...
		// System.out.println(" merged " + mergedPointsSize + " points, old=" +
		// oldPointsCount + ", new=" + newPointsSize);
	...
	static BerkeleyDBOctreeNode fromDB(final Transaction txn,
			final BerkeleyDBOctree octree, final byte[] id, final byte[] data,
		...
		final Geodetic3D averagePoint = ByteBufferUtils
				.getGeodetic3D(byteBuffer);

		final byte formatID = byteBuffer.get();
		final Format format = Format.getFromID(formatID);

		final BerkeleyDBOctreeNode tile = new BerkeleyDBOctreeNode(octree, id,
				sector, pointsCount, averagePoint, format);
		if (loadPoints) {
	...
	private List<Geodetic3D> loadPoints(final Transaction txn) {
		...
		final DatabaseEntry dataEntry = new DatabaseEntry();
		final OperationStatus status = nodeDataDB.get(txn, new DatabaseEntry(
				_id), dataEntry, LockMode.DEFAULT);
		if (status != OperationStatus.SUCCESS) {
			throw new RuntimeException("Unsupported status=" + status);
		}
		...
		final List<Geodetic3D> points = ByteBufferUtils.getPoints(byteBuffer,
				_format, _pointsCount);
