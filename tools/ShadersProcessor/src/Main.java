...
public class Main {
	...
	private static void performConversion(String shadersFolder, String filePath) {
		...
		String classText = _classSource.replace("ADDING_SHADERS",
				addingShadersString);
	...
	private static boolean needsConversion(String shadersFolder, String filePath) {
		long tf = new File(filePath).lastModified();

		final File folder = new File(shadersFolder);

		if (folder.lastModified() > tf) {
			return true;
		}

		for (final File fileEntry : folder.listFiles()) {
			if (fileEntry.lastModified() > tf) {
				return true;
			}
		}
		return false;
	...
	public static void main(final String[] args) {
		...
		final String filePath = sdkProjectPath
				+ "/Commons/Basic/BasicShadersGL2.hpp";
		...
		t.scheduleAtFixedRate(new TimerTask() {
			...
			@Override
			public void run() {
				if (needsConversion(shadersFolder, filePath)){
					performConversion(shadersFolder, filePath);
				}
