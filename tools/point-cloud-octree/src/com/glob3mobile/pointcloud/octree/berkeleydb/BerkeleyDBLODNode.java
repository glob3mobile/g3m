...
public class BerkeleyDBLODNode implements PersistentLOD.Node {

   private static class BerkeleyDBLODNodeLevel implements PersistentLOD.NodeLevel {

      private static BerkeleyDBLODNodeLevel fromDB(final com.sleepycat.je.Transaction txn, final BerkeleyDBLOD db, final Format format, final byte[] id,
                                                   final int lodLevel, final boolean loadPoints, final int pointsCount, final Geodetic3D averagePoint) {
         if (loadPoints) {
      ...
      static private List<Geodetic3D> loadPoints(final com.sleepycat.je.Transaction txn, final BerkeleyDBLOD db, final byte[] id, final int lodLevel,
                                                 final Format format, final int pointsCount, final Geodetic3D averagePoint) {
      ...
      private static DatabaseEntry createNodeLevelDataKey(final byte[] id, final int lodLevel) {
      ...
      private void save(final com.sleepycat.je.Transaction txn, final Format format) {
      ...
      public String toString() {
         return "[BerkeleyDBLODNodeLevel id=" + Utils.toIDString(_id) + ", lodLevel=" + _lodLevel + ", points=" + _pointsCount + ", averagePoint="
               + _averagePoint + "]";
      }

   }

   static BerkeleyDBLODNode create(final BerkeleyDBLOD db, final byte[] id, final List<List<Geodetic3D>> levelsPoints) {
   ...
   static BerkeleyDBLODNode fromDB(final com.sleepycat.je.Transaction txn, final BerkeleyDBLOD db, final byte[] id, final byte[] data,
   ...
   public List<PersistentLOD.NodeLevel> getLevels() {
      return Collections.unmodifiableList(new ArrayList<>(_levels));
   }
