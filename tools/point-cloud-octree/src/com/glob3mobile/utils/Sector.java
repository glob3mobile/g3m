...
public class Sector implements Serializable {
	...
	public Sector(final Geodetic2D lower, final Geodetic2D upper,
	...
	public Sector(final Angle lowerLatitude, final Angle lowerLongitude,
			final Angle upperLatitude, final Angle upperLongitude,
			final double minHeight, final double maxHeight) {
	...
	public String toString() {
		return "[Sector " + _lower + " / " + _upper + ". minHeight: "
				+ _minHeight + "/ maxHeight: " + _maxHeight + "]";
	}

	public final double getUCoordinate(final Angle longitude) {
		return (longitude._radians - _lower._longitude._radians)
				/ _deltaLongitude._radians;
	}

	public final double getVCoordinate(final Angle latitude) {
		return (_upper._latitude._radians - latitude._radians)
				/ _deltaLatitude._radians;
	}
	...
	public int hashCode() {
		...
		result = (int) ((prime * result) + _minHeight);
		result = (int) ((prime * result) + _maxHeight);
		return result;
	}
	...
	public boolean equals(final Object obj) {
		...
		if (_minHeight != other._minHeight) {
			return false;
		}
		if (_maxHeight != other._maxHeight) {
			return false;
		}
		return true;
	}
	...
	public final Sector mergedWith(final Sector that) {
		...
		final Angle lowLon = Angle.min(_lower._longitude,
				that._lower._longitude);
		final Geodetic2D low = new Geodetic2D(lowLat, lowLon);

		final Angle upLat = Angle.max(_upper._latitude, that._upper._latitude);
		final Angle upLon = Angle
				.max(_upper._longitude, that._upper._longitude);
		final Geodetic2D up = new Geodetic2D(upLat, upLon);
		
		final double minHeight = Math.min(_minHeight, that._minHeight);
		final double maxHeight = Math.max(_maxHeight, that._maxHeight);

		return new Sector(low, up, minHeight, maxHeight);
	}
	...
	public final boolean touchesWith(final Sector that) {
		...
		if (_minHeight > that._maxHeight || _maxHeight < that._minHeight) {
			return false;
		}
	...
	public final Sector intersection(final Sector that) {
		...
		final Angle lowLon = Angle.max(_lower._longitude,
				that._lower._longitude);

		final Angle upLat = Angle.min(_upper._latitude, that._upper._latitude);
		final Angle upLon = Angle
				.min(_upper._longitude, that._upper._longitude);
		
		final double minHeight = Math.max(_minHeight, that._minHeight);
		final double maxHeight = Math.min(_maxHeight, that._maxHeight);

		if (lowLat.lowerThan(upLat) && lowLon.lowerThan(upLon) && (minHeight < maxHeight)) {
			final Geodetic2D low = new Geodetic2D(lowLat, lowLon);
			final Geodetic2D up = new Geodetic2D(upLat, upLon);
